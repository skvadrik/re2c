/* Generated by re2c */
#line 1 "conditions/condtype_forwdecl.cs.re"
enum cond_t : int;

int main ()
{
	cond_t cond;
	char * YYCURSOR;
#define YYGETCONDITION() cond

#line 12 "conditions/condtype_forwdecl.cs.c"
{
	unsigned char yych;
	if (YYGETCONDITION() < 1) {
		goto yyc_a;
	} else {
		goto yyc_b;
	}
/* *********************************** */
yyc_a:
	yych = *YYCURSOR;
	if (yych == 'a') goto yy4;
yy4:
	++YYCURSOR;
#line 12 "conditions/condtype_forwdecl.cs.re"
	{}
#line 28 "conditions/condtype_forwdecl.cs.c"
/* *********************************** */
yyc_b:
	yych = *YYCURSOR;
	if (yych == 'b') goto yy9;
yy9:
	++YYCURSOR;
#line 13 "conditions/condtype_forwdecl.cs.re"
	{}
#line 37 "conditions/condtype_forwdecl.cs.c"
}
#line 14 "conditions/condtype_forwdecl.cs.re"

	return 0;
}

#line 44 "conditions/condtype_forwdecl.cs.c"
enum cond_t : int {
	yyca,
	yycb,
};
#line 18 "conditions/condtype_forwdecl.cs.re"

re2c: warning: line 14: control flow in condition 'a' is undefined for strings that match '[\x0-\x60\x62-\xFF]', use default rule '*' [-Wundefined-control-flow]
re2c: warning: line 14: control flow in condition 'b' is undefined for strings that match '[\x0-\x61\x63-\xFF]', use default rule '*' [-Wundefined-control-flow]
