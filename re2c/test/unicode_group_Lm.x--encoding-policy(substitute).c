/* Generated by re2c */
#line 1 "unicode_group_Lm.x--encoding-policy(substitute).re"
#include <stdio.h>
#define YYCTYPE unsigned short
bool scan(const YYCTYPE * start, const YYCTYPE * const limit)
{
	__attribute__((unused)) const YYCTYPE * YYMARKER; // silence compiler warnings when YYMARKER is not used
#	define YYCURSOR start
Lm:
	
#line 12 "<stdout>"
{
	YYCTYPE yych;

	yych = *YYCURSOR;
	if (yych <= 0x1D9A) {
		if (yych <= 0x0819) {
			if (yych <= 0x0374) {
				if (yych <= 0x02E4) {
					if (yych <= 0x02C5) {
						if (yych <= 0x02AF) goto yy5;
						if (yych <= 0x02C1) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x02D1) goto yy3;
						if (yych <= 0x02DF) goto yy5;
						goto yy3;
					}
				} else {
					if (yych <= 0x02ED) {
						if (yych == 0x02EC) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x02EE) goto yy3;
						if (yych <= 0x0373) goto yy5;
						goto yy3;
					}
				}
			} else {
				if (yych <= 0x0640) {
					if (yych <= 0x0558) {
						if (yych == 0x037A) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x0559) goto yy3;
						if (yych <= 0x063F) goto yy5;
						goto yy3;
					}
				} else {
					if (yych <= 0x07F3) {
						if (yych <= 0x06E4) goto yy5;
						if (yych <= 0x06E6) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x07F5) goto yy3;
						if (yych == 0x07FA) goto yy3;
						goto yy5;
					}
				}
			}
		} else {
			if (yych <= 0x10FC) {
				if (yych <= 0x0970) {
					if (yych <= 0x0824) {
						if (yych <= 0x081A) goto yy3;
						if (yych <= 0x0823) goto yy5;
						goto yy3;
					} else {
						if (yych == 0x0828) goto yy3;
						goto yy5;
					}
				} else {
					if (yych <= 0x0E46) {
						if (yych <= 0x0971) goto yy3;
						if (yych <= 0x0E45) goto yy5;
						goto yy3;
					} else {
						if (yych == 0x0EC6) goto yy3;
						if (yych <= 0x10FB) goto yy5;
						goto yy3;
					}
				}
			} else {
				if (yych <= 0x1AA7) {
					if (yych <= 0x1842) {
						if (yych == 0x17D7) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x1843) goto yy3;
						if (yych <= 0x1AA6) goto yy5;
						goto yy3;
					}
				} else {
					if (yych <= 0x1D2B) {
						if (yych <= 0x1C77) goto yy5;
						if (yych <= 0x1C7D) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0x1D61) goto yy3;
						if (yych == 0x1D78) goto yy3;
						goto yy5;
					}
				}
			}
		}
	} else {
		if (yych <= 0xA015) {
			if (yych <= 0x2E2E) {
				if (yych <= 0x208F) {
					if (yych <= 0x2071) {
						if (yych <= 0x1DBF) goto yy3;
						if (yych <= 0x2070) goto yy5;
						goto yy3;
					} else {
						if (yych == 0x207F) goto yy3;
						goto yy5;
					}
				} else {
					if (yych <= 0x2C7D) {
						if (yych <= 0x209C) goto yy3;
						if (yych <= 0x2C7C) goto yy5;
						goto yy3;
					} else {
						if (yych == 0x2D6F) goto yy3;
						goto yy5;
					}
				}
			} else {
				if (yych <= 0x303A) {
					if (yych <= 0x3005) {
						if (yych <= 0x2E2F) goto yy3;
						if (yych <= 0x3004) goto yy5;
						goto yy3;
					} else {
						if (yych <= 0x3030) goto yy5;
						if (yych <= 0x3035) goto yy3;
						goto yy5;
					}
				} else {
					if (yych <= 0x309E) {
						if (yych <= 0x303B) goto yy3;
						if (yych <= 0x309C) goto yy5;
						goto yy3;
					} else {
						if (yych <= 0x30FB) goto yy5;
						if (yych <= 0x30FE) goto yy3;
						if (yych <= 0xA014) goto yy5;
						goto yy3;
					}
				}
			}
		} else {
			if (yych <= 0xA9CE) {
				if (yych <= 0xA67F) {
					if (yych <= 0xA60B) {
						if (yych <= 0xA4F7) goto yy5;
						if (yych <= 0xA4FD) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0xA60C) goto yy3;
						if (yych <= 0xA67E) goto yy5;
						goto yy3;
					}
				} else {
					if (yych <= 0xA76F) {
						if (yych <= 0xA716) goto yy5;
						if (yych <= 0xA71F) goto yy3;
						goto yy5;
					} else {
						if (yych <= 0xA770) goto yy3;
						if (yych == 0xA788) goto yy3;
						goto yy5;
					}
				}
			} else {
				if (yych <= 0xD7FF) {
					if (yych <= 0xAA70) {
						if (yych <= 0xA9CF) goto yy3;
						if (yych <= 0xAA6F) goto yy5;
						goto yy3;
					} else {
						if (yych == 0xAADD) goto yy3;
						goto yy5;
					}
				} else {
					if (yych <= 0xFF6F) {
						if (yych <= 0xDBFF) goto yy7;
						if (yych >= 0xE000) goto yy5;
					} else {
						if (yych <= 0xFF70) goto yy3;
						if (yych <= 0xFF9D) goto yy5;
						if (yych <= 0xFF9F) goto yy3;
						goto yy5;
					}
				}
			}
		}
	}
yy2:
yy3:
	++YYCURSOR;
#line 11 "unicode_group_Lm.x--encoding-policy(substitute).re"
	{ goto Lm; }
#line 205 "<stdout>"
yy5:
	++YYCURSOR;
#line 12 "unicode_group_Lm.x--encoding-policy(substitute).re"
	{ return YYCURSOR == limit; }
#line 210 "<stdout>"
yy7:
	++YYCURSOR;
	if ((yych = *YYCURSOR) <= 0xDBFF) goto yy2;
	if (yych <= 0xDFFF) goto yy5;
	goto yy2;
}
#line 13 "unicode_group_Lm.x--encoding-policy(substitute).re"

}
static const char buffer_Lm [] = "";
int main ()
{
	if (!scan (reinterpret_cast<const YYCTYPE *> (buffer_Lm), reinterpret_cast<const YYCTYPE *> (buffer_Lm + sizeof (buffer_Lm) - 1)))
		printf("test 'Lm' failed\n");
}
