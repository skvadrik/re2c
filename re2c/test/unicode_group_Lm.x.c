/* Generated by re2c */
#line 1 "unicode_group_Lm.x.re"
#include <stdio.h>
#define YYCTYPE unsigned short
bool scan(const YYCTYPE * start, const YYCTYPE * const limit)
{
	__attribute__((unused)) const YYCTYPE * YYMARKER; // silence compiler warnings when YYMARKER is not used
#	define YYCURSOR start
Lm:
	
#line 12 "<stdout>"
{
	YYCTYPE yych;

	yych = *YYCURSOR;
	if (yych <= 0x1D78) {
		if (yych <= 0x0819) {
			if (yych <= 0x0374) {
				if (yych <= 0x02E4) {
					if (yych <= 0x02C5) {
						if (yych <= 0x02AF) goto yy4;
						if (yych >= 0x02C2) goto yy4;
					} else {
						if (yych <= 0x02D1) goto yy2;
						if (yych <= 0x02DF) goto yy4;
					}
				} else {
					if (yych <= 0x02ED) {
						if (yych != 0x02EC) goto yy4;
					} else {
						if (yych <= 0x02EE) goto yy2;
						if (yych <= 0x0373) goto yy4;
					}
				}
			} else {
				if (yych <= 0x0640) {
					if (yych <= 0x0558) {
						if (yych != 0x037A) goto yy4;
					} else {
						if (yych <= 0x0559) goto yy2;
						if (yych <= 0x063F) goto yy4;
					}
				} else {
					if (yych <= 0x07F3) {
						if (yych <= 0x06E4) goto yy4;
						if (yych >= 0x06E7) goto yy4;
					} else {
						if (yych <= 0x07F5) goto yy2;
						if (yych != 0x07FA) goto yy4;
					}
				}
			}
		} else {
			if (yych <= 0x10FB) {
				if (yych <= 0x0970) {
					if (yych <= 0x0824) {
						if (yych <= 0x081A) goto yy2;
						if (yych <= 0x0823) goto yy4;
					} else {
						if (yych != 0x0828) goto yy4;
					}
				} else {
					if (yych <= 0x0E46) {
						if (yych <= 0x0971) goto yy2;
						if (yych <= 0x0E45) goto yy4;
					} else {
						if (yych != 0x0EC6) goto yy4;
					}
				}
			} else {
				if (yych <= 0x1AA6) {
					if (yych <= 0x17D7) {
						if (yych <= 0x10FC) goto yy2;
						if (yych <= 0x17D6) goto yy4;
					} else {
						if (yych != 0x1843) goto yy4;
					}
				} else {
					if (yych <= 0x1C7D) {
						if (yych <= 0x1AA7) goto yy2;
						if (yych <= 0x1C77) goto yy4;
					} else {
						if (yych <= 0x1D2B) goto yy4;
						if (yych <= 0x1D61) goto yy2;
						if (yych <= 0x1D77) goto yy4;
					}
				}
			}
		}
	} else {
		if (yych <= 0xA014) {
			if (yych <= 0x2D6F) {
				if (yych <= 0x207F) {
					if (yych <= 0x2070) {
						if (yych <= 0x1D9A) goto yy4;
						if (yych >= 0x1DC0) goto yy4;
					} else {
						if (yych <= 0x2071) goto yy2;
						if (yych <= 0x207E) goto yy4;
					}
				} else {
					if (yych <= 0x2C7C) {
						if (yych <= 0x208F) goto yy4;
						if (yych >= 0x209D) goto yy4;
					} else {
						if (yych <= 0x2C7D) goto yy2;
						if (yych <= 0x2D6E) goto yy4;
					}
				}
			} else {
				if (yych <= 0x3035) {
					if (yych <= 0x3004) {
						if (yych != 0x2E2F) goto yy4;
					} else {
						if (yych <= 0x3005) goto yy2;
						if (yych <= 0x3030) goto yy4;
					}
				} else {
					if (yych <= 0x309C) {
						if (yych != 0x303B) goto yy4;
					} else {
						if (yych <= 0x309E) goto yy2;
						if (yych <= 0x30FB) goto yy4;
						if (yych >= 0x30FF) goto yy4;
					}
				}
			}
		} else {
			if (yych <= 0xA788) {
				if (yych <= 0xA67E) {
					if (yych <= 0xA4FD) {
						if (yych <= 0xA015) goto yy2;
						if (yych <= 0xA4F7) goto yy4;
					} else {
						if (yych != 0xA60C) goto yy4;
					}
				} else {
					if (yych <= 0xA71F) {
						if (yych <= 0xA67F) goto yy2;
						if (yych <= 0xA716) goto yy4;
					} else {
						if (yych == 0xA770) goto yy2;
						if (yych <= 0xA787) goto yy4;
					}
				}
			} else {
				if (yych <= 0xAADD) {
					if (yych <= 0xAA6F) {
						if (yych != 0xA9CF) goto yy4;
					} else {
						if (yych <= 0xAA70) goto yy2;
						if (yych <= 0xAADC) goto yy4;
					}
				} else {
					if (yych <= 0xFF6F) {
						if (yych <= 0xD7FF) goto yy4;
						if (yych <= 0xDBFF) goto yy6;
						goto yy4;
					} else {
						if (yych <= 0xFF70) goto yy2;
						if (yych <= 0xFF9D) goto yy4;
						if (yych >= 0xFFA0) goto yy4;
					}
				}
			}
		}
	}
yy2:
	++YYCURSOR;
#line 11 "unicode_group_Lm.x.re"
	{ goto Lm; }
#line 173 "<stdout>"
yy4:
	++YYCURSOR;
yy5:
#line 12 "unicode_group_Lm.x.re"
	{ return YYCURSOR == limit; }
#line 179 "<stdout>"
yy6:
	++YYCURSOR;
	if ((yych = *YYCURSOR) <= 0xDBFF) goto yy5;
	if (yych <= 0xDFFF) goto yy4;
	goto yy5;
}
#line 13 "unicode_group_Lm.x.re"

}
static const char buffer_Lm [] = "";
int main ()
{
	if (!scan (reinterpret_cast<const YYCTYPE *> (buffer_Lm), reinterpret_cast<const YYCTYPE *> (buffer_Lm + sizeof (buffer_Lm) - 1)))
		printf("test 'Lm' failed\n");
}
