/* Generated by re2c */
#line 1 "encodings/unicode_group_N_x_encoding_policy_fail.re"
// re2c $INPUT -o $OUTPUT -x --encoding-policy fail
#include <stdint.h>
#include <stdio.h>
#include "utf16.h"
#define YYCTYPE uint16_t

bool scan(const YYCTYPE* start, const YYCTYPE* const limit) {
	__attribute__((unused)) const YYCTYPE* YYMARKER;
#define YYCURSOR start
N:

#line 15 "encodings/unicode_group_N_x_encoding_policy_fail.c"
{
	YYCTYPE yych;
	yych = *YYCURSOR;
	if (yych <= 0x2070) {
		if (yych <= 0x0D78) {
			if (yych <= 0x09F9) {
				if (yych <= 0x0669) {
					if (yych <= 0x00B8) {
						if (yych <= '9') {
							if (yych >= '0') goto yy3;
						} else {
							if (yych <= 0x00B1) goto yy1;
							if (yych <= 0x00B3) goto yy3;
						}
					} else {
						if (yych <= 0x00BB) {
							if (yych <= 0x00B9) goto yy3;
						} else {
							if (yych <= 0x00BE) goto yy3;
							if (yych >= 0x0660) goto yy3;
						}
					}
				} else {
					if (yych <= 0x0965) {
						if (yych <= 0x06F9) {
							if (yych >= 0x06F0) goto yy3;
						} else {
							if (yych <= 0x07BF) goto yy1;
							if (yych <= 0x07C9) goto yy3;
						}
					} else {
						if (yych <= 0x09E5) {
							if (yych <= 0x096F) goto yy3;
						} else {
							if (yych <= 0x09EF) goto yy3;
							if (yych >= 0x09F4) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x0BF2) {
					if (yych <= 0x0B65) {
						if (yych <= 0x0A6F) {
							if (yych >= 0x0A66) goto yy3;
						} else {
							if (yych <= 0x0AE5) goto yy1;
							if (yych <= 0x0AEF) goto yy3;
						}
					} else {
						if (yych <= 0x0B71) {
							if (yych <= 0x0B6F) goto yy3;
						} else {
							if (yych <= 0x0B77) goto yy3;
							if (yych >= 0x0BE6) goto yy3;
						}
					}
				} else {
					if (yych <= 0x0CE5) {
						if (yych <= 0x0C6F) {
							if (yych >= 0x0C66) goto yy3;
						} else {
							if (yych <= 0x0C77) goto yy1;
							if (yych <= 0x0C7E) goto yy3;
						}
					} else {
						if (yych <= 0x0D57) {
							if (yych <= 0x0CEF) goto yy3;
						} else {
							if (yych <= 0x0D5E) goto yy3;
							if (yych >= 0x0D66) goto yy3;
						}
					}
				}
			}
		} else {
			if (yych <= 0x17F9) {
				if (yych <= 0x1049) {
					if (yych <= 0x0ECF) {
						if (yych <= 0x0DEF) {
							if (yych >= 0x0DE6) goto yy3;
						} else {
							if (yych <= 0x0E4F) goto yy1;
							if (yych <= 0x0E59) goto yy3;
						}
					} else {
						if (yych <= 0x0F1F) {
							if (yych <= 0x0ED9) goto yy3;
						} else {
							if (yych <= 0x0F33) goto yy3;
							if (yych >= 0x1040) goto yy3;
						}
					}
				} else {
					if (yych <= 0x16ED) {
						if (yych <= 0x1099) {
							if (yych >= 0x1090) goto yy3;
						} else {
							if (yych <= 0x1368) goto yy1;
							if (yych <= 0x137C) goto yy3;
						}
					} else {
						if (yych <= 0x17DF) {
							if (yych <= 0x16F0) goto yy3;
						} else {
							if (yych <= 0x17E9) goto yy3;
							if (yych >= 0x17F0) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x1A99) {
					if (yych <= 0x19CF) {
						if (yych <= 0x1819) {
							if (yych >= 0x1810) goto yy3;
						} else {
							if (yych <= 0x1945) goto yy1;
							if (yych <= 0x194F) goto yy3;
						}
					} else {
						if (yych <= 0x1A7F) {
							if (yych <= 0x19DA) goto yy3;
						} else {
							if (yych <= 0x1A89) goto yy3;
							if (yych >= 0x1A90) goto yy3;
						}
					}
				} else {
					if (yych <= 0x1C3F) {
						if (yych <= 0x1B59) {
							if (yych >= 0x1B50) goto yy3;
						} else {
							if (yych <= 0x1BAF) goto yy1;
							if (yych <= 0x1BB9) goto yy3;
						}
					} else {
						if (yych <= 0x1C4F) {
							if (yych <= 0x1C49) goto yy3;
						} else {
							if (yych <= 0x1C59) goto yy3;
							if (yych >= 0x2070) goto yy3;
						}
					}
				}
			}
		}
	} else {
		if (yych <= 0xA835) {
			if (yych <= 0x3029) {
				if (yych <= 0x249B) {
					if (yych <= 0x214F) {
						if (yych <= 0x2079) {
							if (yych >= 0x2074) goto yy3;
						} else {
							if (yych <= 0x207F) goto yy1;
							if (yych <= 0x2089) goto yy3;
						}
					} else {
						if (yych <= 0x2184) {
							if (yych <= 0x2182) goto yy3;
						} else {
							if (yych <= 0x2189) goto yy3;
							if (yych >= 0x2460) goto yy3;
						}
					}
				} else {
					if (yych <= 0x2CFC) {
						if (yych <= 0x24FF) {
							if (yych >= 0x24EA) goto yy3;
						} else {
							if (yych <= 0x2775) goto yy1;
							if (yych <= 0x2793) goto yy3;
						}
					} else {
						if (yych <= 0x3006) {
							if (yych <= 0x2CFD) goto yy3;
						} else {
							if (yych <= 0x3007) goto yy3;
							if (yych >= 0x3021) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x325F) {
					if (yych <= 0x321F) {
						if (yych <= 0x303A) {
							if (yych >= 0x3038) goto yy3;
						} else {
							if (yych <= 0x3191) goto yy1;
							if (yych <= 0x3195) goto yy3;
						}
					} else {
						if (yych <= 0x3247) {
							if (yych <= 0x3229) goto yy3;
						} else {
							if (yych != 0x3250) goto yy3;
						}
					}
				} else {
					if (yych <= 0xA61F) {
						if (yych <= 0x3289) {
							if (yych >= 0x3280) goto yy3;
						} else {
							if (yych <= 0x32B0) goto yy1;
							if (yych <= 0x32BF) goto yy3;
						}
					} else {
						if (yych <= 0xA6E5) {
							if (yych <= 0xA629) goto yy3;
						} else {
							if (yych <= 0xA6EF) goto yy3;
							if (yych >= 0xA830) goto yy3;
						}
					}
				}
			}
		} else {
			if (yych <= 0xD806) {
				if (yych <= 0xAA59) {
					if (yych <= 0xA9CF) {
						if (yych <= 0xA8D9) {
							if (yych >= 0xA8D0) goto yy3;
						} else {
							if (yych <= 0xA8FF) goto yy1;
							if (yych <= 0xA909) goto yy3;
						}
					} else {
						if (yych <= 0xA9EF) {
							if (yych <= 0xA9D9) goto yy3;
						} else {
							if (yych <= 0xA9F9) goto yy3;
							if (yych >= 0xAA50) goto yy3;
						}
					}
				} else {
					if (yych <= 0xD801) {
						if (yych <= 0xABF9) {
							if (yych >= 0xABF0) goto yy3;
						} else {
							if (yych <= 0xD7FF) goto yy1;
							if (yych <= 0xD800) goto yy4;
							goto yy5;
						}
					} else {
						if (yych <= 0xD803) {
							if (yych <= 0xD802) goto yy6;
							goto yy7;
						} else {
							if (yych <= 0xD804) goto yy8;
							if (yych <= 0xD805) goto yy9;
							goto yy10;
						}
					}
				}
			} else {
				if (yych <= 0xD837) {
					if (yych <= 0xD81A) {
						if (yych <= 0xD808) {
							if (yych <= 0xD807) goto yy11;
						} else {
							if (yych <= 0xD809) goto yy12;
							if (yych >= 0xD81A) goto yy13;
						}
					} else {
						if (yych <= 0xD833) {
							if (yych <= 0xD81B) goto yy14;
						} else {
							if (yych <= 0xD834) goto yy15;
							if (yych <= 0xD835) goto yy16;
						}
					}
				} else {
					if (yych <= 0xD83C) {
						if (yych <= 0xD839) {
							if (yych <= 0xD838) goto yy17;
							goto yy18;
						} else {
							if (yych <= 0xD83A) goto yy19;
							if (yych <= 0xD83B) goto yy20;
							goto yy21;
						}
					} else {
						if (yych <= 0xD83E) {
							if (yych >= 0xD83E) goto yy22;
						} else {
							if (yych <= 0xFF0F) goto yy1;
							if (yych <= 0xFF19) goto yy3;
						}
					}
				}
			}
		}
	}
yy1:
	++YYCURSOR;
yy2:
#line 15 "encodings/unicode_group_N_x_encoding_policy_fail.re"
	{ return YYCURSOR - 1 == limit; }
#line 313 "encodings/unicode_group_N_x_encoding_policy_fail.c"
yy3:
	++YYCURSOR;
#line 14 "encodings/unicode_group_N_x_encoding_policy_fail.re"
	{ goto N; }
#line 318 "encodings/unicode_group_N_x_encoding_policy_fail.c"
yy4:
	yych = *++YYCURSOR;
	if (yych <= 0xDEFB) {
		if (yych <= 0xDD78) {
			if (yych <= 0xDD06) goto yy2;
			if (yych <= 0xDD33) goto yy3;
			if (yych <= 0xDD3F) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDD89) goto yy2;
			if (yych <= 0xDD8B) goto yy3;
			if (yych <= 0xDEE0) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDF41) {
			if (yych <= 0xDF1F) goto yy2;
			if (yych <= 0xDF23) goto yy3;
			if (yych <= 0xDF40) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDF4A) {
				if (yych <= 0xDF49) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDFD0) goto yy2;
				if (yych <= 0xDFD5) goto yy3;
				goto yy2;
			}
		}
	}
yy5:
	yych = *++YYCURSOR;
	if (yych <= 0xDC9F) goto yy2;
	if (yych <= 0xDCA9) goto yy3;
	goto yy2;
yy6:
	yych = *++YYCURSOR;
	if (yych <= 0xDDD1) {
		if (yych <= 0xDCFA) {
			if (yych <= 0xDC78) {
				if (yych <= 0xDC57) goto yy2;
				if (yych <= 0xDC5F) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDC7F) goto yy3;
				if (yych <= 0xDCA6) goto yy2;
				if (yych <= 0xDCAF) goto yy3;
				goto yy2;
			}
		} else {
			if (yych <= 0xDDBB) {
				if (yych <= 0xDCFF) goto yy3;
				if (yych <= 0xDD15) goto yy2;
				if (yych <= 0xDD1B) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDDBD) goto yy3;
				if (yych <= 0xDDBF) goto yy2;
				if (yych <= 0xDDCF) goto yy3;
				goto yy2;
			}
		}
	} else {
		if (yych <= 0xDEEA) {
			if (yych <= 0xDE7C) {
				if (yych <= 0xDDFF) goto yy3;
				if (yych <= 0xDE3F) goto yy2;
				if (yych <= 0xDE48) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDE7E) goto yy3;
				if (yych <= 0xDE9C) goto yy2;
				if (yych <= 0xDE9F) goto yy3;
				goto yy2;
			}
		} else {
			if (yych <= 0xDF77) {
				if (yych <= 0xDEEF) goto yy3;
				if (yych <= 0xDF57) goto yy2;
				if (yych <= 0xDF5F) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDF7F) goto yy3;
				if (yych <= 0xDFA8) goto yy2;
				if (yych <= 0xDFAF) goto yy3;
				goto yy2;
			}
		}
	}
yy7:
	yych = *++YYCURSOR;
	if (yych <= 0xDE7E) {
		if (yych <= 0xDD2F) {
			if (yych <= 0xDCF9) goto yy2;
			if (yych <= 0xDCFF) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDD39) goto yy3;
			if (yych <= 0xDE5F) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDF50) {
			if (yych <= 0xDF1C) goto yy2;
			if (yych <= 0xDF26) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDF54) goto yy3;
			if (yych <= 0xDFC4) goto yy2;
			if (yych <= 0xDFCB) goto yy3;
			goto yy2;
		}
	}
yy8:
	yych = *++YYCURSOR;
	if (yych <= 0xDD3F) {
		if (yych <= 0xDCEF) {
			if (yych <= 0xDC51) goto yy2;
			if (yych <= 0xDC6F) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDCF9) goto yy3;
			if (yych <= 0xDD35) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDDE0) {
			if (yych <= 0xDDCF) goto yy2;
			if (yych <= 0xDDD9) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDDF4) goto yy3;
			if (yych <= 0xDEEF) goto yy2;
			if (yych <= 0xDEF9) goto yy3;
			goto yy2;
		}
	}
yy9:
	yych = *++YYCURSOR;
	if (yych <= 0xDE4F) {
		if (yych <= 0xDC59) {
			if (yych <= 0xDC4F) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDCCF) goto yy2;
			if (yych <= 0xDCD9) goto yy3;
			goto yy2;
		}
	} else {
		if (yych <= 0xDEC9) {
			if (yych <= 0xDE59) goto yy3;
			if (yych <= 0xDEBF) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDF2F) goto yy2;
			if (yych <= 0xDF3B) goto yy3;
			goto yy2;
		}
	}
yy10:
	yych = *++YYCURSOR;
	if (yych <= 0xDCDF) goto yy2;
	if (yych <= 0xDCF2) goto yy3;
	if (yych <= 0xDD4F) goto yy2;
	if (yych <= 0xDD59) goto yy3;
	goto yy2;
yy11:
	yych = *++YYCURSOR;
	if (yych <= 0xDD9F) {
		if (yych <= 0xDC6C) {
			if (yych <= 0xDC4F) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDD4F) goto yy2;
			if (yych <= 0xDD59) goto yy3;
			goto yy2;
		}
	} else {
		if (yych <= 0xDF59) {
			if (yych <= 0xDDA9) goto yy3;
			if (yych <= 0xDF4F) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDFBF) goto yy2;
			if (yych <= 0xDFD4) goto yy3;
			goto yy2;
		}
	}
yy12:
	yych = *++YYCURSOR;
	if (yych <= 0xDBFF) goto yy2;
	if (yych <= 0xDC6E) goto yy3;
	goto yy2;
yy13:
	yych = *++YYCURSOR;
	if (yych <= 0xDEC9) {
		if (yych <= 0xDE5F) goto yy2;
		if (yych <= 0xDE69) goto yy3;
		if (yych <= 0xDEBF) goto yy2;
		goto yy3;
	} else {
		if (yych <= 0xDF59) {
			if (yych <= 0xDF4F) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDF5A) goto yy2;
			if (yych <= 0xDF61) goto yy3;
			goto yy2;
		}
	}
yy14:
	yych = *++YYCURSOR;
	if (yych <= 0xDE7F) goto yy2;
	if (yych <= 0xDE96) goto yy3;
	goto yy2;
yy15:
	yych = *++YYCURSOR;
	if (yych <= 0xDEDF) {
		if (yych <= 0xDEBF) goto yy2;
		if (yych <= 0xDED3) goto yy3;
		goto yy2;
	} else {
		if (yych <= 0xDEF3) goto yy3;
		if (yych <= 0xDF5F) goto yy2;
		if (yych <= 0xDF78) goto yy3;
		goto yy2;
	}
yy16:
	yych = *++YYCURSOR;
	if (yych <= 0xDFCD) goto yy2;
	if (yych <= 0xDFFF) goto yy3;
	goto yy2;
yy17:
	yych = *++YYCURSOR;
	if (yych <= 0xDD3F) goto yy2;
	if (yych <= 0xDD49) goto yy3;
	if (yych <= 0xDEEF) goto yy2;
	if (yych <= 0xDEF9) goto yy3;
	goto yy2;
yy18:
	yych = *++YYCURSOR;
	if (yych <= 0xDCEF) goto yy2;
	if (yych <= 0xDCF9) goto yy3;
	goto yy2;
yy19:
	yych = *++YYCURSOR;
	if (yych <= 0xDCC6) goto yy2;
	if (yych <= 0xDCCF) goto yy3;
	if (yych <= 0xDD4F) goto yy2;
	if (yych <= 0xDD59) goto yy3;
	goto yy2;
yy20:
	yych = *++YYCURSOR;
	if (yych <= 0xDCB0) {
		if (yych <= 0xDCAB) {
			if (yych <= 0xDC70) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDCAC) goto yy2;
			if (yych <= 0xDCAF) goto yy3;
			goto yy2;
		}
	} else {
		if (yych <= 0xDD2D) {
			if (yych <= 0xDCB4) goto yy3;
			if (yych <= 0xDD00) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDD2E) goto yy2;
			if (yych <= 0xDD3D) goto yy3;
			goto yy2;
		}
	}
yy21:
	yych = *++YYCURSOR;
	if (yych <= 0xDCFF) goto yy2;
	if (yych <= 0xDD0C) goto yy3;
	goto yy2;
yy22:
	yych = *++YYCURSOR;
	if (yych <= 0xDFEF) goto yy2;
	if (yych <= 0xDFF9) goto yy3;
	goto yy2;
}
#line 16 "encodings/unicode_group_N_x_encoding_policy_fail.re"

}

static const uint32_t chars_N[] = {0x30,0x39,0xb2,0xb3,0xb9,0xb9,0xbc,0xbe,0x660,0x669,0x6f0,0x6f9,0x7c0,0x7c9,0x966,0x96f,0x9e6,0x9ef,0x9f4,0x9f9,0xa66,0xa6f,0xae6,0xaef,0xb66,0xb6f,0xb72,0xb77,0xbe6,0xbf2,0xc66,0xc6f,0xc78,0xc7e,0xce6,0xcef,0xd58,0xd5e,0xd66,0xd78,0xde6,0xdef,0xe50,0xe59,0xed0,0xed9,0xf20,0xf33,0x1040,0x1049,0x1090,0x1099,0x1369,0x137c,0x16ee,0x16f0,0x17e0,0x17e9,0x17f0,0x17f9,0x1810,0x1819,0x1946,0x194f,0x19d0,0x19da,0x1a80,0x1a89,0x1a90,0x1a99,0x1b50,0x1b59,0x1bb0,0x1bb9,0x1c40,0x1c49,0x1c50,0x1c59,0x2070,0x2070,0x2074,0x2079,0x2080,0x2089,0x2150,0x2182,0x2185,0x2189,0x2460,0x249b,0x24ea,0x24ff,0x2776,0x2793,0x2cfd,0x2cfd,0x3007,0x3007,0x3021,0x3029,0x3038,0x303a,0x3192,0x3195,0x3220,0x3229,0x3248,0x324f,0x3251,0x325f,0x3280,0x3289,0x32b1,0x32bf,0xa620,0xa629,0xa6e6,0xa6ef,0xa830,0xa835,0xa8d0,0xa8d9,0xa900,0xa909,0xa9d0,0xa9d9,0xa9f0,0xa9f9,0xaa50,0xaa59,0xabf0,0xabf9,0xff10,0xff19,0x10107,0x10133,0x10140,0x10178,0x1018a,0x1018b,0x102e1,0x102fb,0x10320,0x10323,0x10341,0x10341,0x1034a,0x1034a,0x103d1,0x103d5,0x104a0,0x104a9,0x10858,0x1085f,0x10879,0x1087f,0x108a7,0x108af,0x108fb,0x108ff,0x10916,0x1091b,0x109bc,0x109bd,0x109c0,0x109cf,0x109d2,0x109ff,0x10a40,0x10a48,0x10a7d,0x10a7e,0x10a9d,0x10a9f,0x10aeb,0x10aef,0x10b58,0x10b5f,0x10b78,0x10b7f,0x10ba9,0x10baf,0x10cfa,0x10cff,0x10d30,0x10d39,0x10e60,0x10e7e,0x10f1d,0x10f26,0x10f51,0x10f54,0x10fc5,0x10fcb,0x11052,0x1106f,0x110f0,0x110f9,0x11136,0x1113f,0x111d0,0x111d9,0x111e1,0x111f4,0x112f0,0x112f9,0x11450,0x11459,0x114d0,0x114d9,0x11650,0x11659,0x116c0,0x116c9,0x11730,0x1173b,0x118e0,0x118f2,0x11950,0x11959,0x11c50,0x11c6c,0x11d50,0x11d59,0x11da0,0x11da9,0x11f50,0x11f59,0x11fc0,0x11fd4,0x12400,0x1246e,0x16a60,0x16a69,0x16ac0,0x16ac9,0x16b50,0x16b59,0x16b5b,0x16b61,0x16e80,0x16e96,0x1d2c0,0x1d2d3,0x1d2e0,0x1d2f3,0x1d360,0x1d378,0x1d7ce,0x1d7ff,0x1e140,0x1e149,0x1e2f0,0x1e2f9,0x1e4f0,0x1e4f9,0x1e8c7,0x1e8cf,0x1e950,0x1e959,0x1ec71,0x1ecab,0x1ecad,0x1ecaf,0x1ecb1,0x1ecb4,0x1ed01,0x1ed2d,0x1ed2f,0x1ed3d,0x1f100,0x1f10c,0x1fbf0,0x1fbf9,};

static uint32_t encode_utf16(const uint32_t* ranges, uint32_t ranges_count, uint16_t* s) {
	uint16_t* const s0 = s;
	for (uint32_t i = 0; i < ranges_count; i += 2)
		for (uint32_t j = ranges[i]; j <= ranges[i + 1]; ++j)
			if (j <= re2c::utf16::MAX_1WORD_RUNE) {
				*s++ = j;
			} else {
				*s++ = re2c::utf16::lead_surr(j);
				*s++ = re2c::utf16::trail_surr(j);
			}
	for (uint32_t i = 0; i < 2; ++i) s[i] = 0;
	return s - s0;
}

int main() {
	YYCTYPE* buffer_N = new YYCTYPE[3664];
	uint32_t buffer_N_len = encode_utf16(chars_N, sizeof(chars_N) / sizeof(uint32_t), buffer_N);
	if (!scan(buffer_N, buffer_N + buffer_N_len)) printf("test 'N' failed\n");
	delete[] buffer_N;
	return 0;
}
