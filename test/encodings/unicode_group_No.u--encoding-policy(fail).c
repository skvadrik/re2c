/* Generated by re2c */
#line 1 "encodings/unicode_group_No.u--encoding-policy(fail).re"
#include <stdio.h>

#define YYCTYPE unsigned int
bool scan(const YYCTYPE * start, const YYCTYPE * const limit)
{
	__attribute__((unused)) const YYCTYPE * YYMARKER; // silence compiler warnings when YYMARKER is not used
#	define YYCURSOR start
No:
	
#line 13 "encodings/unicode_group_No.u--encoding-policy(fail).c"
{
	YYCTYPE yych;
	yych = *YYCURSOR;
	if (yych <= 0x00003289) {
		if (yych <= 0x00002070) {
			if (yych <= 0x00000C77) {
				if (yych <= 0x000000BE) {
					if (yych <= 0x000000B8) {
						if (yych <= 0x000000B1) goto yy2;
						if (yych <= 0x000000B3) goto yy4;
					} else {
						if (yych <= 0x000000B9) goto yy4;
						if (yych >= 0x000000BC) goto yy4;
					}
				} else {
					if (yych <= 0x00000B71) {
						if (yych <= 0x000009F3) goto yy2;
						if (yych <= 0x000009F9) goto yy4;
					} else {
						if (yych <= 0x00000B77) goto yy4;
						if (yych <= 0x00000BEF) goto yy2;
						if (yych <= 0x00000BF2) goto yy4;
					}
				}
			} else {
				if (yych <= 0x00001368) {
					if (yych <= 0x00000D75) {
						if (yych <= 0x00000C7E) goto yy4;
						if (yych >= 0x00000D70) goto yy4;
					} else {
						if (yych <= 0x00000F29) goto yy2;
						if (yych <= 0x00000F33) goto yy4;
					}
				} else {
					if (yych <= 0x000017F9) {
						if (yych <= 0x0000137C) goto yy4;
						if (yych >= 0x000017F0) goto yy4;
					} else {
						if (yych == 0x000019DA) goto yy4;
						if (yych >= 0x00002070) goto yy4;
					}
				}
			}
		} else {
			if (yych <= 0x00002775) {
				if (yych <= 0x0000215F) {
					if (yych <= 0x0000207F) {
						if (yych <= 0x00002073) goto yy2;
						if (yych <= 0x00002079) goto yy4;
					} else {
						if (yych <= 0x00002089) goto yy4;
						if (yych >= 0x00002150) goto yy4;
					}
				} else {
					if (yych <= 0x0000245F) {
						if (yych == 0x00002189) goto yy4;
					} else {
						if (yych <= 0x0000249B) goto yy4;
						if (yych <= 0x000024E9) goto yy2;
						if (yych <= 0x000024FF) goto yy4;
					}
				}
			} else {
				if (yych <= 0x0000321F) {
					if (yych <= 0x00002CFD) {
						if (yych <= 0x00002793) goto yy4;
						if (yych >= 0x00002CFD) goto yy4;
					} else {
						if (yych <= 0x00003191) goto yy2;
						if (yych <= 0x00003195) goto yy4;
					}
				} else {
					if (yych <= 0x0000324F) {
						if (yych <= 0x00003229) goto yy4;
						if (yych >= 0x00003248) goto yy4;
					} else {
						if (yych <= 0x00003250) goto yy2;
						if (yych <= 0x0000325F) goto yy4;
						if (yych >= 0x00003280) goto yy4;
					}
				}
			}
		}
	} else {
		if (yych <= 0x00010A7E) {
			if (yych <= 0x0001031F) {
				if (yych <= 0x00010133) {
					if (yych <= 0x0000A82F) {
						if (yych <= 0x000032B0) goto yy2;
						if (yych <= 0x000032BF) goto yy4;
					} else {
						if (yych <= 0x0000A835) goto yy4;
						if (yych >= 0x00010107) goto yy4;
					}
				} else {
					if (yych <= 0x00010189) {
						if (yych <= 0x00010174) goto yy2;
						if (yych <= 0x00010178) goto yy4;
					} else {
						if (yych <= 0x0001018B) goto yy4;
						if (yych <= 0x000102E0) goto yy2;
						if (yych <= 0x000102FB) goto yy4;
					}
				}
			} else {
				if (yych <= 0x000108A6) {
					if (yych <= 0x0001085F) {
						if (yych <= 0x00010323) goto yy4;
						if (yych >= 0x00010858) goto yy4;
					} else {
						if (yych <= 0x00010878) goto yy2;
						if (yych <= 0x0001087F) goto yy4;
					}
				} else {
					if (yych <= 0x0001091B) {
						if (yych <= 0x000108AF) goto yy4;
						if (yych >= 0x00010916) goto yy4;
					} else {
						if (yych <= 0x00010A3F) goto yy2;
						if (yych <= 0x00010A47) goto yy4;
						if (yych >= 0x00010A7D) goto yy4;
					}
				}
			}
		} else {
			if (yych <= 0x00011051) {
				if (yych <= 0x00010B5F) {
					if (yych <= 0x00010AEA) {
						if (yych <= 0x00010A9C) goto yy2;
						if (yych <= 0x00010A9F) goto yy4;
					} else {
						if (yych <= 0x00010AEF) goto yy4;
						if (yych >= 0x00010B58) goto yy4;
					}
				} else {
					if (yych <= 0x00010BA8) {
						if (yych <= 0x00010B77) goto yy2;
						if (yych <= 0x00010B7F) goto yy4;
					} else {
						if (yych <= 0x00010BAF) goto yy4;
						if (yych <= 0x00010E5F) goto yy2;
						if (yych <= 0x00010E7E) goto yy4;
					}
				}
			} else {
				if (yych <= 0x00016B61) {
					if (yych <= 0x000111F4) {
						if (yych <= 0x00011065) goto yy4;
						if (yych >= 0x000111E1) goto yy4;
					} else {
						if (yych <= 0x000118E9) goto yy2;
						if (yych <= 0x000118F2) goto yy4;
						if (yych >= 0x00016B5B) goto yy4;
					}
				} else {
					if (yych <= 0x0001E8C6) {
						if (yych <= 0x0001D35F) goto yy2;
						if (yych <= 0x0001D371) goto yy4;
					} else {
						if (yych <= 0x0001E8CF) goto yy4;
						if (yych <= 0x0001F0FF) goto yy2;
						if (yych <= 0x0001F10C) goto yy4;
					}
				}
			}
		}
	}
yy2:
	++YYCURSOR;
#line 13 "encodings/unicode_group_No.u--encoding-policy(fail).re"
	{ return YYCURSOR == limit; }
#line 185 "encodings/unicode_group_No.u--encoding-policy(fail).c"
yy4:
	++YYCURSOR;
#line 12 "encodings/unicode_group_No.u--encoding-policy(fail).re"
	{ goto No; }
#line 190 "encodings/unicode_group_No.u--encoding-policy(fail).c"
}
#line 14 "encodings/unicode_group_No.u--encoding-policy(fail).re"

}
static const unsigned int chars_No [] = {0xb2,0xb3,  0xb9,0xb9,  0xbc,0xbe,  0x9f4,0x9f9,  0xb72,0xb77,  0xbf0,0xbf2,  0xc78,0xc7e,  0xd70,0xd75,  0xf2a,0xf33,  0x1369,0x137c,  0x17f0,0x17f9,  0x19da,0x19da,  0x2070,0x2070,  0x2074,0x2079,  0x2080,0x2089,  0x2150,0x215f,  0x2189,0x2189,  0x2460,0x249b,  0x24ea,0x24ff,  0x2776,0x2793,  0x2cfd,0x2cfd,  0x3192,0x3195,  0x3220,0x3229,  0x3248,0x324f,  0x3251,0x325f,  0x3280,0x3289,  0x32b1,0x32bf,  0xa830,0xa835,  0x10107,0x10133,  0x10175,0x10178,  0x1018a,0x1018b,  0x102e1,0x102fb,  0x10320,0x10323,  0x10858,0x1085f,  0x10879,0x1087f,  0x108a7,0x108af,  0x10916,0x1091b,  0x10a40,0x10a47,  0x10a7d,0x10a7e,  0x10a9d,0x10a9f,  0x10aeb,0x10aef,  0x10b58,0x10b5f,  0x10b78,0x10b7f,  0x10ba9,0x10baf,  0x10e60,0x10e7e,  0x11052,0x11065,  0x111e1,0x111f4,  0x118ea,0x118f2,  0x16b5b,0x16b61,  0x1d360,0x1d371,  0x1e8c7,0x1e8cf,  0x1f100,0x1f10c,  0x0,0x0};
static unsigned int encode_utf32 (const unsigned int * ranges, unsigned int ranges_count, unsigned int * s)
{
	unsigned int * const s_start = s;
	for (unsigned int i = 0; i < ranges_count; i += 2)
		for (unsigned int j = ranges[i]; j <= ranges[i + 1]; ++j)
			*s++ = j;
	return s - s_start;
}

int main ()
{
	unsigned int * buffer_No = new unsigned int [571];
	YYCTYPE * s = (YYCTYPE *) buffer_No;
	unsigned int buffer_len = encode_utf32 (chars_No, sizeof (chars_No) / sizeof (unsigned int), buffer_No);
	/* convert 32-bit code units to YYCTYPE; reuse the same buffer */
	for (unsigned int i = 0; i < buffer_len; ++i) s[i] = buffer_No[i];
	if (!scan (s, s + buffer_len))
		printf("test 'No' failed\n");
	delete [] buffer_No;
	return 0;
}
