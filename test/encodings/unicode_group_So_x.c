/* Generated by re2c */
#line 1 "encodings/unicode_group_So_x.re"
// re2c $INPUT -o $OUTPUT -x
#include <stdint.h>
#include <stdio.h>
#include "utf16.h"
#define YYCTYPE uint16_t

bool scan(const YYCTYPE* start, const YYCTYPE* const limit) {
	__attribute__((unused)) const YYCTYPE* YYMARKER;
#define YYCURSOR start
So:

#line 15 "encodings/unicode_group_So_x.c"
{
	YYCTYPE yych;
	yych = *YYCURSOR;
	if (yych <= 0x237C) {
		if (yych <= 0x166D) {
			if (yych <= 0x0C7F) {
				if (yych <= 0x06DE) {
					if (yych <= 0x00B0) {
						if (yych <= 0x00A9) {
							if (yych == 0x00A6) goto yy3;
							if (yych >= 0x00A9) goto yy3;
						} else {
							if (yych == 0x00AE) goto yy3;
							if (yych >= 0x00B0) goto yy3;
						}
					} else {
						if (yych <= 0x058E) {
							if (yych == 0x0482) goto yy3;
							if (yych >= 0x058D) goto yy3;
						} else {
							if (yych <= 0x060D) goto yy1;
							if (yych <= 0x060F) goto yy3;
							if (yych >= 0x06DE) goto yy3;
						}
					}
				} else {
					if (yych <= 0x09FA) {
						if (yych <= 0x06FE) {
							if (yych == 0x06E9) goto yy3;
							if (yych >= 0x06FD) goto yy3;
						} else {
							if (yych == 0x07F6) goto yy3;
							if (yych >= 0x09FA) goto yy3;
						}
					} else {
						if (yych <= 0x0BF8) {
							if (yych == 0x0B70) goto yy3;
							if (yych >= 0x0BF3) goto yy3;
						} else {
							if (yych == 0x0BFA) goto yy3;
							if (yych >= 0x0C7F) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x0F36) {
					if (yych <= 0x0F13) {
						if (yych <= 0x0D79) {
							if (yych == 0x0D4F) goto yy3;
							if (yych >= 0x0D79) goto yy3;
						} else {
							if (yych <= 0x0F00) goto yy1;
							if (yych <= 0x0F03) goto yy3;
							if (yych >= 0x0F13) goto yy3;
						}
					} else {
						if (yych <= 0x0F1F) {
							if (yych <= 0x0F14) goto yy1;
							if (yych <= 0x0F17) goto yy3;
							if (yych >= 0x0F1A) goto yy3;
						} else {
							if (yych == 0x0F34) goto yy3;
							if (yych >= 0x0F36) goto yy3;
						}
					}
				} else {
					if (yych <= 0x0FCF) {
						if (yych <= 0x0FC5) {
							if (yych == 0x0F38) goto yy3;
							if (yych >= 0x0FBE) goto yy3;
						} else {
							if (yych <= 0x0FC6) goto yy1;
							if (yych != 0x0FCD) goto yy3;
						}
					} else {
						if (yych <= 0x109F) {
							if (yych <= 0x0FD4) goto yy1;
							if (yych <= 0x0FD8) goto yy3;
							if (yych >= 0x109E) goto yy3;
						} else {
							if (yych <= 0x138F) goto yy1;
							if (yych <= 0x1399) goto yy3;
							if (yych >= 0x166D) goto yy3;
						}
					}
				}
			}
		} else {
			if (yych <= 0x214A) {
				if (yych <= 0x2114) {
					if (yych <= 0x1B7C) {
						if (yych <= 0x19FF) {
							if (yych == 0x1940) goto yy3;
							if (yych >= 0x19DE) goto yy3;
						} else {
							if (yych <= 0x1B60) goto yy1;
							if (yych <= 0x1B6A) goto yy3;
							if (yych >= 0x1B74) goto yy3;
						}
					} else {
						if (yych <= 0x2106) {
							if (yych <= 0x20FF) goto yy1;
							if (yych != 0x2102) goto yy3;
						} else {
							if (yych <= 0x2107) goto yy1;
							if (yych <= 0x2109) goto yy3;
							if (yych >= 0x2114) goto yy3;
						}
					}
				} else {
					if (yych <= 0x2127) {
						if (yych <= 0x2123) {
							if (yych <= 0x2115) goto yy1;
							if (yych <= 0x2117) goto yy3;
							if (yych >= 0x211E) goto yy3;
						} else {
							if (yych == 0x2125) goto yy3;
							if (yych >= 0x2127) goto yy3;
						}
					} else {
						if (yych <= 0x212E) {
							if (yych == 0x2129) goto yy3;
							if (yych >= 0x212E) goto yy3;
						} else {
							if (yych <= 0x2139) goto yy1;
							if (yych <= 0x213B) goto yy3;
							if (yych >= 0x214A) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x21AD) {
					if (yych <= 0x2199) {
						if (yych <= 0x214F) {
							if (yych <= 0x214B) goto yy1;
							if (yych != 0x214E) goto yy3;
						} else {
							if (yych <= 0x2189) goto yy1;
							if (yych <= 0x218B) goto yy3;
							if (yych >= 0x2195) goto yy3;
						}
					} else {
						if (yych <= 0x21A2) {
							if (yych <= 0x219B) goto yy1;
							if (yych != 0x21A0) goto yy3;
						} else {
							if (yych <= 0x21A3) goto yy1;
							if (yych != 0x21A6) goto yy3;
						}
					}
				} else {
					if (yych <= 0x21F3) {
						if (yych <= 0x21D1) {
							if (yych <= 0x21AE) goto yy1;
							if (yych <= 0x21CD) goto yy3;
							if (yych >= 0x21D0) goto yy3;
						} else {
							if (yych == 0x21D3) goto yy3;
							if (yych >= 0x21D5) goto yy3;
						}
					} else {
						if (yych <= 0x231F) {
							if (yych <= 0x22FF) goto yy1;
							if (yych <= 0x2307) goto yy3;
							if (yych >= 0x230C) goto yy3;
						} else {
							if (yych <= 0x2328) {
								if (yych >= 0x2322) goto yy3;
							} else {
								if (yych <= 0x232A) goto yy1;
								if (yych <= 0x237B) goto yy3;
							}
						}
					}
				}
			}
		}
	} else {
		if (yych <= 0x3229) {
			if (yych <= 0x2CE4) {
				if (yych <= 0x25FF) {
					if (yych <= 0x249B) {
						if (yych <= 0x23E1) {
							if (yych <= 0x239A) goto yy3;
							if (yych <= 0x23B3) goto yy1;
							if (yych <= 0x23DB) goto yy3;
						} else {
							if (yych <= 0x2429) goto yy3;
							if (yych <= 0x243F) goto yy1;
							if (yych <= 0x244A) goto yy3;
						}
					} else {
						if (yych <= 0x25B7) {
							if (yych <= 0x24E9) goto yy3;
							if (yych <= 0x24FF) goto yy1;
							if (yych <= 0x25B6) goto yy3;
						} else {
							if (yych == 0x25C1) goto yy1;
							if (yych <= 0x25F7) goto yy3;
						}
					}
				} else {
					if (yych <= 0x2AFF) {
						if (yych <= 0x2793) {
							if (yych == 0x266F) goto yy1;
							if (yych <= 0x2767) goto yy3;
						} else {
							if (yych <= 0x27BF) goto yy3;
							if (yych <= 0x27FF) goto yy1;
							if (yych <= 0x28FF) goto yy3;
						}
					} else {
						if (yych <= 0x2B4C) {
							if (yych <= 0x2B2F) goto yy3;
							if (yych <= 0x2B44) goto yy1;
							if (yych <= 0x2B46) goto yy3;
						} else {
							if (yych <= 0x2B73) goto yy3;
							if (yych <= 0x2B75) goto yy1;
							if (yych <= 0x2BFF) goto yy3;
						}
					}
				}
			} else {
				if (yych <= 0x301F) {
					if (yych <= 0x2EFF) {
						if (yych <= 0x2E7F) {
							if (yych <= 0x2CEA) goto yy3;
							if (yych <= 0x2E4F) goto yy1;
							if (yych <= 0x2E51) goto yy3;
						} else {
							if (yych == 0x2E9A) goto yy1;
							if (yych <= 0x2EF3) goto yy3;
						}
					} else {
						if (yych <= 0x3003) {
							if (yych <= 0x2FD5) goto yy3;
							if (yych <= 0x2FEF) goto yy1;
							if (yych <= 0x2FFF) goto yy3;
						} else {
							if (yych <= 0x3004) goto yy3;
							if (yych <= 0x3011) goto yy1;
							if (yych <= 0x3013) goto yy3;
						}
					}
				} else {
					if (yych <= 0x3195) {
						if (yych <= 0x303D) {
							if (yych <= 0x3020) goto yy3;
							if (yych <= 0x3035) goto yy1;
							if (yych <= 0x3037) goto yy3;
						} else {
							if (yych <= 0x303F) goto yy3;
							if (yych <= 0x318F) goto yy1;
							if (yych <= 0x3191) goto yy3;
						}
					} else {
						if (yych <= 0x31EE) {
							if (yych <= 0x319F) goto yy3;
							if (yych <= 0x31BF) goto yy1;
							if (yych <= 0x31E5) goto yy3;
						} else {
							if (yych <= 0x31EF) goto yy3;
							if (yych <= 0x31FF) goto yy1;
							if (yych <= 0x321E) goto yy3;
						}
					}
				}
			}
		} else {
			if (yych <= 0xD81A) {
				if (yych <= 0xA835) {
					if (yych <= 0x32BF) {
						if (yych <= 0x325F) {
							if (yych <= 0x3247) goto yy3;
							if (yych == 0x3250) goto yy3;
						} else {
							if (yych <= 0x327F) goto yy3;
							if (yych <= 0x3289) goto yy1;
							if (yych <= 0x32B0) goto yy3;
						}
					} else {
						if (yych <= 0xA48F) {
							if (yych <= 0x33FF) goto yy3;
							if (yych <= 0x4DBF) goto yy1;
							if (yych <= 0x4DFF) goto yy3;
						} else {
							if (yych <= 0xA4C6) goto yy3;
							if (yych <= 0xA827) goto yy1;
							if (yych <= 0xA82B) goto yy3;
						}
					}
				} else {
					if (yych <= 0xD801) {
						if (yych <= 0xAA76) {
							if (yych == 0xA838) goto yy1;
							if (yych <= 0xA839) goto yy3;
						} else {
							if (yych <= 0xAA79) goto yy3;
							if (yych == 0xD800) goto yy4;
						}
					} else {
						if (yych <= 0xD805) {
							if (yych <= 0xD802) goto yy5;
							if (yych <= 0xD803) goto yy6;
							if (yych >= 0xD805) goto yy7;
						} else {
							if (yych == 0xD807) goto yy8;
							if (yych >= 0xD81A) goto yy9;
						}
					}
				}
			} else {
				if (yych <= 0xFBD2) {
					if (yych <= 0xD837) {
						if (yych <= 0xD833) {
							if (yych == 0xD82F) goto yy10;
							if (yych >= 0xD833) goto yy11;
						} else {
							if (yych <= 0xD834) goto yy12;
							if (yych == 0xD836) goto yy13;
						}
					} else {
						if (yych <= 0xD83C) {
							if (yych <= 0xD838) goto yy14;
							if (yych <= 0xD83A) goto yy1;
							if (yych <= 0xD83B) goto yy15;
							goto yy16;
						} else {
							if (yych <= 0xD83D) goto yy17;
							if (yych <= 0xD83E) goto yy18;
							if (yych >= 0xFBC3) goto yy3;
						}
					}
				} else {
					if (yych <= 0xFDFF) {
						if (yych <= 0xFD91) {
							if (yych <= 0xFD3F) goto yy1;
							if (yych <= 0xFD4F) goto yy3;
							if (yych >= 0xFD90) goto yy3;
						} else {
							if (yych <= 0xFDC7) goto yy1;
							if (yych <= 0xFDCF) goto yy3;
							if (yych >= 0xFDFD) goto yy3;
						}
					} else {
						if (yych <= 0xFFE8) {
							if (yych == 0xFFE4) goto yy3;
							if (yych >= 0xFFE8) goto yy3;
						} else {
							if (yych <= 0xFFEE) {
								if (yych >= 0xFFED) goto yy3;
							} else {
								if (yych <= 0xFFFB) goto yy1;
								if (yych <= 0xFFFD) goto yy3;
							}
						}
					}
				}
			}
		}
	}
yy1:
	++YYCURSOR;
yy2:
#line 15 "encodings/unicode_group_So_x.re"
	{ return YYCURSOR - 1 == limit; }
#line 383 "encodings/unicode_group_So_x.c"
yy3:
	++YYCURSOR;
#line 14 "encodings/unicode_group_So_x.re"
	{ goto So; }
#line 388 "encodings/unicode_group_So_x.c"
yy4:
	yych = *++YYCURSOR;
	if (yych <= 0xDD8E) {
		if (yych <= 0xDD78) {
			if (yych <= 0xDD36) goto yy2;
			if (yych <= 0xDD3F) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDD89) goto yy3;
			if (yych <= 0xDD8B) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDD9F) {
			if (yych <= 0xDD8F) goto yy2;
			if (yych <= 0xDD9C) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDDA0) goto yy3;
			if (yych <= 0xDDCF) goto yy2;
			if (yych <= 0xDDFC) goto yy3;
			goto yy2;
		}
	}
yy5:
	yych = *++YYCURSOR;
	if (yych <= 0xDC76) goto yy2;
	if (yych <= 0xDC78) goto yy3;
	if (yych == 0xDEC8) goto yy3;
	goto yy2;
yy6:
	yych = *++YYCURSOR;
	if (yych <= 0xDED0) goto yy2;
	if (yych <= 0xDED8) goto yy3;
	goto yy2;
yy7:
	yych = *++YYCURSOR;
	if (yych == 0xDF3F) goto yy3;
	goto yy2;
yy8:
	yych = *++YYCURSOR;
	if (yych <= 0xDFD4) goto yy2;
	if (yych <= 0xDFDC) goto yy3;
	if (yych <= 0xDFE0) goto yy2;
	if (yych <= 0xDFF1) goto yy3;
	goto yy2;
yy9:
	yych = *++YYCURSOR;
	if (yych <= 0xDF3B) goto yy2;
	if (yych <= 0xDF3F) goto yy3;
	if (yych == 0xDF45) goto yy3;
	goto yy2;
yy10:
	yych = *++YYCURSOR;
	if (yych == 0xDC9C) goto yy3;
	goto yy2;
yy11:
	yych = *++YYCURSOR;
	if (yych <= 0xDEB3) {
		if (yych <= 0xDCF9) {
			if (yych <= 0xDBFF) goto yy2;
			if (yych <= 0xDCEF) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDCFC) goto yy3;
			if (yych <= 0xDCFF) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDEDF) {
			if (yych <= 0xDEB9) goto yy2;
			if (yych <= 0xDED0) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDEEF) goto yy3;
			if (yych <= 0xDF4F) goto yy2;
			if (yych <= 0xDFC3) goto yy3;
			goto yy2;
		}
	}
yy12:
	yych = *++YYCURSOR;
	if (yych <= 0xDD84) {
		if (yych <= 0xDD28) {
			if (yych <= 0xDCF5) {
				if (yych <= 0xDBFF) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDCFF) goto yy2;
				if (yych <= 0xDD26) goto yy3;
				goto yy2;
			}
		} else {
			if (yych <= 0xDD69) {
				if (yych <= 0xDD64) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDD6C) goto yy3;
				if (yych <= 0xDD82) goto yy2;
				goto yy3;
			}
		}
	} else {
		if (yych <= 0xDDFF) {
			if (yych <= 0xDDA9) {
				if (yych <= 0xDD8B) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDDAD) goto yy2;
				if (yych <= 0xDDEA) goto yy3;
				goto yy2;
			}
		} else {
			if (yych <= 0xDE45) {
				if (yych <= 0xDE41) goto yy3;
				if (yych <= 0xDE44) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDEFF) goto yy2;
				if (yych <= 0xDF56) goto yy3;
				goto yy2;
			}
		}
	}
yy13:
	yych = *++YYCURSOR;
	if (yych <= 0xDE6C) {
		if (yych <= 0xDDFF) {
			if (yych <= 0xDBFF) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDE36) goto yy2;
			if (yych <= 0xDE3A) goto yy3;
			goto yy2;
		}
	} else {
		if (yych <= 0xDE83) {
			if (yych == 0xDE75) goto yy2;
			goto yy3;
		} else {
			if (yych <= 0xDE84) goto yy2;
			if (yych <= 0xDE86) goto yy3;
			goto yy2;
		}
	}
yy14:
	yych = *++YYCURSOR;
	if (yych == 0xDD4F) goto yy3;
	goto yy2;
yy15:
	yych = *++YYCURSOR;
	if (yych == 0xDCAC) goto yy3;
	if (yych == 0xDD2E) goto yy3;
	goto yy2;
yy16:
	yych = *++YYCURSOR;
	if (yych <= 0xDD0C) {
		if (yych <= 0xDCAE) {
			if (yych <= 0xDC2F) {
				if (yych <= 0xDBFF) goto yy2;
				if (yych <= 0xDC2B) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDC93) goto yy3;
				if (yych <= 0xDC9F) goto yy2;
				goto yy3;
			}
		} else {
			if (yych <= 0xDCC0) {
				if (yych <= 0xDCB0) goto yy2;
				if (yych <= 0xDCBF) goto yy3;
				goto yy2;
			} else {
				if (yych == 0xDCD0) goto yy2;
				if (yych <= 0xDCF5) goto yy3;
				goto yy2;
			}
		}
	} else {
		if (yych <= 0xDE48) {
			if (yych <= 0xDE02) {
				if (yych <= 0xDDAD) goto yy3;
				if (yych <= 0xDDE5) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDE0F) goto yy2;
				if (yych <= 0xDE3B) goto yy3;
				if (yych <= 0xDE3F) goto yy2;
				goto yy3;
			}
		} else {
			if (yych <= 0xDE5F) {
				if (yych <= 0xDE4F) goto yy2;
				if (yych <= 0xDE51) goto yy3;
				goto yy2;
			} else {
				if (yych <= 0xDE65) goto yy3;
				if (yych <= 0xDEFF) goto yy2;
				if (yych <= 0xDFFA) goto yy3;
				goto yy2;
			}
		}
	}
yy17:
	yych = *++YYCURSOR;
	if (yych <= 0xDEFC) {
		if (yych <= 0xDEDB) {
			if (yych <= 0xDBFF) goto yy2;
			if (yych <= 0xDED8) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDEEC) goto yy3;
			if (yych <= 0xDEEF) goto yy2;
			goto yy3;
		}
	} else {
		if (yych <= 0xDFDF) {
			if (yych <= 0xDEFF) goto yy2;
			if (yych <= 0xDFD9) goto yy3;
			goto yy2;
		} else {
			if (yych <= 0xDFEB) goto yy3;
			if (yych == 0xDFF0) goto yy3;
			goto yy2;
		}
	}
yy18:
	yych = *++YYCURSOR;
	if (yych <= 0xDE6F) {
		if (yych <= 0xDC8F) {
			if (yych <= 0xDC47) {
				if (yych <= 0xDBFF) goto yy2;
				if (yych <= 0xDC0B) goto yy3;
				if (yych <= 0xDC0F) goto yy2;
				goto yy3;
			} else {
				if (yych <= 0xDC59) {
					if (yych <= 0xDC4F) goto yy2;
					goto yy3;
				} else {
					if (yych <= 0xDC5F) goto yy2;
					if (yych <= 0xDC87) goto yy3;
					goto yy2;
				}
			}
		} else {
			if (yych <= 0xDCC1) {
				if (yych <= 0xDCAF) {
					if (yych <= 0xDCAD) goto yy3;
					goto yy2;
				} else {
					if (yych <= 0xDCBB) goto yy3;
					if (yych <= 0xDCBF) goto yy2;
					goto yy3;
				}
			} else {
				if (yych <= 0xDE57) {
					if (yych <= 0xDCFF) goto yy2;
					goto yy3;
				} else {
					if (yych <= 0xDE5F) goto yy2;
					if (yych <= 0xDE6D) goto yy3;
					goto yy2;
				}
			}
		}
	} else {
		if (yych <= 0xDEDE) {
			if (yych <= 0xDEC6) {
				if (yych <= 0xDE7F) {
					if (yych <= 0xDE7C) goto yy3;
					goto yy2;
				} else {
					if (yych <= 0xDE8A) goto yy3;
					if (yych <= 0xDE8D) goto yy2;
					goto yy3;
				}
			} else {
				if (yych <= 0xDEC8) {
					if (yych <= 0xDEC7) goto yy2;
					goto yy3;
				} else {
					if (yych <= 0xDECC) goto yy2;
					if (yych <= 0xDEDC) goto yy3;
					goto yy2;
				}
			}
		} else {
			if (yych <= 0xDF92) {
				if (yych <= 0xDEEE) {
					if (yych <= 0xDEEA) goto yy3;
					goto yy2;
				} else {
					if (yych <= 0xDEF8) goto yy3;
					if (yych <= 0xDEFF) goto yy2;
					goto yy3;
				}
			} else {
				if (yych <= 0xDFEF) {
					if (yych <= 0xDF93) goto yy2;
					goto yy3;
				} else {
					if (yych == 0xDFFA) goto yy3;
					goto yy2;
				}
			}
		}
	}
}
#line 16 "encodings/unicode_group_So_x.re"

}

static const uint32_t chars_So[] = {0xa6,0xa6,0xa9,0xa9,0xae,0xae,0xb0,0xb0,0x482,0x482,0x58d,0x58e,0x60e,0x60f,0x6de,0x6de,0x6e9,0x6e9,0x6fd,0x6fe,0x7f6,0x7f6,0x9fa,0x9fa,0xb70,0xb70,0xbf3,0xbf8,0xbfa,0xbfa,0xc7f,0xc7f,0xd4f,0xd4f,0xd79,0xd79,0xf01,0xf03,0xf13,0xf13,0xf15,0xf17,0xf1a,0xf1f,0xf34,0xf34,0xf36,0xf36,0xf38,0xf38,0xfbe,0xfc5,0xfc7,0xfcc,0xfce,0xfcf,0xfd5,0xfd8,0x109e,0x109f,0x1390,0x1399,0x166d,0x166d,0x1940,0x1940,0x19de,0x19ff,0x1b61,0x1b6a,0x1b74,0x1b7c,0x2100,0x2101,0x2103,0x2106,0x2108,0x2109,0x2114,0x2114,0x2116,0x2117,0x211e,0x2123,0x2125,0x2125,0x2127,0x2127,0x2129,0x2129,0x212e,0x212e,0x213a,0x213b,0x214a,0x214a,0x214c,0x214d,0x214f,0x214f,0x218a,0x218b,0x2195,0x2199,0x219c,0x219f,0x21a1,0x21a2,0x21a4,0x21a5,0x21a7,0x21ad,0x21af,0x21cd,0x21d0,0x21d1,0x21d3,0x21d3,0x21d5,0x21f3,0x2300,0x2307,0x230c,0x231f,0x2322,0x2328,0x232b,0x237b,0x237d,0x239a,0x23b4,0x23db,0x23e2,0x2429,0x2440,0x244a,0x249c,0x24e9,0x2500,0x25b6,0x25b8,0x25c0,0x25c2,0x25f7,0x2600,0x266e,0x2670,0x2767,0x2794,0x27bf,0x2800,0x28ff,0x2b00,0x2b2f,0x2b45,0x2b46,0x2b4d,0x2b73,0x2b76,0x2bff,0x2ce5,0x2cea,0x2e50,0x2e51,0x2e80,0x2e99,0x2e9b,0x2ef3,0x2f00,0x2fd5,0x2ff0,0x2fff,0x3004,0x3004,0x3012,0x3013,0x3020,0x3020,0x3036,0x3037,0x303e,0x303f,0x3190,0x3191,0x3196,0x319f,0x31c0,0x31e5,0x31ef,0x31ef,0x3200,0x321e,0x322a,0x3247,0x3250,0x3250,0x3260,0x327f,0x328a,0x32b0,0x32c0,0x33ff,0x4dc0,0x4dff,0xa490,0xa4c6,0xa828,0xa82b,0xa836,0xa837,0xa839,0xa839,0xaa77,0xaa79,0xfbc3,0xfbd2,0xfd40,0xfd4f,0xfd90,0xfd91,0xfdc8,0xfdcf,0xfdfd,0xfdff,0xffe4,0xffe4,0xffe8,0xffe8,0xffed,0xffee,0xfffc,0xfffd,0x10137,0x1013f,0x10179,0x10189,0x1018c,0x1018e,0x10190,0x1019c,0x101a0,0x101a0,0x101d0,0x101fc,0x10877,0x10878,0x10ac8,0x10ac8,0x10ed1,0x10ed8,0x1173f,0x1173f,0x11fd5,0x11fdc,0x11fe1,0x11ff1,0x16b3c,0x16b3f,0x16b45,0x16b45,0x1bc9c,0x1bc9c,0x1cc00,0x1ccef,0x1ccfa,0x1ccfc,0x1cd00,0x1ceb3,0x1ceba,0x1ced0,0x1cee0,0x1ceef,0x1cf50,0x1cfc3,0x1d000,0x1d0f5,0x1d100,0x1d126,0x1d129,0x1d164,0x1d16a,0x1d16c,0x1d183,0x1d184,0x1d18c,0x1d1a9,0x1d1ae,0x1d1ea,0x1d200,0x1d241,0x1d245,0x1d245,0x1d300,0x1d356,0x1d800,0x1d9ff,0x1da37,0x1da3a,0x1da6d,0x1da74,0x1da76,0x1da83,0x1da85,0x1da86,0x1e14f,0x1e14f,0x1ecac,0x1ecac,0x1ed2e,0x1ed2e,0x1f000,0x1f02b,0x1f030,0x1f093,0x1f0a0,0x1f0ae,0x1f0b1,0x1f0bf,0x1f0c1,0x1f0cf,0x1f0d1,0x1f0f5,0x1f10d,0x1f1ad,0x1f1e6,0x1f202,0x1f210,0x1f23b,0x1f240,0x1f248,0x1f250,0x1f251,0x1f260,0x1f265,0x1f300,0x1f3fa,0x1f400,0x1f6d8,0x1f6dc,0x1f6ec,0x1f6f0,0x1f6fc,0x1f700,0x1f7d9,0x1f7e0,0x1f7eb,0x1f7f0,0x1f7f0,0x1f800,0x1f80b,0x1f810,0x1f847,0x1f850,0x1f859,0x1f860,0x1f887,0x1f890,0x1f8ad,0x1f8b0,0x1f8bb,0x1f8c0,0x1f8c1,0x1f900,0x1fa57,0x1fa60,0x1fa6d,0x1fa70,0x1fa7c,0x1fa80,0x1fa8a,0x1fa8e,0x1fac6,0x1fac8,0x1fac8,0x1facd,0x1fadc,0x1fadf,0x1faea,0x1faef,0x1faf8,0x1fb00,0x1fb92,0x1fb94,0x1fbef,0x1fbfa,0x1fbfa,};

static uint32_t encode_utf16(const uint32_t* ranges, uint32_t ranges_count, uint16_t* s) {
	uint16_t* const s0 = s;
	for (uint32_t i = 0; i < ranges_count; i += 2)
		for (uint32_t j = ranges[i]; j <= ranges[i + 1]; ++j)
			if (j <= re2c::utf16::MAX_1WORD_RUNE) {
				*s++ = j;
			} else {
				*s++ = re2c::utf16::lead_surr(j);
				*s++ = re2c::utf16::trail_surr(j);
			}
	for (uint32_t i = 0; i < 2; ++i) s[i] = 0;
	return s - s0;
}

int main() {
	YYCTYPE* buffer_So = new YYCTYPE[14938];
	uint32_t buffer_So_len = encode_utf16(chars_So, sizeof(chars_So) / sizeof(uint32_t), buffer_So);
	if (!scan(buffer_So, buffer_So + buffer_So_len)) printf("test 'So' failed\n");
	delete[] buffer_So;
	return 0;
}
