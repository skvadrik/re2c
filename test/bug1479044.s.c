/* Generated by re2c */
#line 1 "bug1479044.s.re"
// re2c $INPUT -o $OUTPUT -s
#define NULL ((char*) 0)
#define YYCTYPE char
#define YYCURSOR p
#define YYLIMIT p
#define YYMARKER q
#define YYFILL(n)

#include <stdio.h>

char *scan281(char *p)
{
	char *q;
start:

#line 19 "bug1479044.s.c"
{
	YYCTYPE yych;
	unsigned int yyaccept = 0;
	if ((YYLIMIT - YYCURSOR) < 11) YYFILL(11);
	yych = *YYCURSOR;
	if (yych <= 'k') {
		if (yych <= '`') {
			if (yych <= 0x00) goto yy2;
			if (yych <= '/') goto yy4;
			if (yych <= '9') goto yy6;
			goto yy4;
		} else {
			if (yych <= 'c') {
				if (yych <= 'a') goto yy7;
				goto yy8;
			} else {
				if (yych <= 'd') goto yy9;
				if (yych <= 'f') goto yy8;
				goto yy4;
			}
		}
	} else {
		if (yych <= 'r') {
			if (yych <= 'o') {
				if (yych <= 'l') goto yy8;
				goto yy4;
			} else {
				if (yych <= 'p') goto yy10;
				if (yych <= 'q') goto yy4;
				goto yy11;
			}
		} else {
			if (yych <= 'v') {
				if (yych <= 'u') goto yy4;
				goto yy8;
			} else {
				if (yych == 'x') goto yy12;
				goto yy4;
			}
		}
	}
yy2:
	++YYCURSOR;
#line 38 "bug1479044.s.re"
	{
		return NULL;
	}
#line 67 "bug1479044.s.c"
yy4:
	++YYCURSOR;
yy5:
#line 33 "bug1479044.s.re"
	{
		goto start;
	}
#line 75 "bug1479044.s.c"
yy6:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych == '-') goto yy13;
	if (yych <= '/') goto yy5;
	if (yych <= '9') goto yy15;
	goto yy5;
yy7:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych == 'd') goto yy17;
	goto yy5;
yy8:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych == 'd') goto yy18;
	goto yy5;
yy9:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych <= 'n') {
		if (yych == 'h') goto yy19;
		goto yy5;
	} else {
		if (yych <= 'o') goto yy20;
		if (yych == 's') goto yy21;
		goto yy5;
	}
yy10:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych <= 'p') {
		if (yych <= 'n') goto yy5;
		if (yych <= 'o') goto yy22;
		goto yy23;
	} else {
		if (yych == 'r') goto yy24;
		goto yy5;
	}
yy11:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych == 'h') goto yy25;
	goto yy5;
yy12:
	yyaccept = 0;
	yych = *(YYMARKER = ++YYCURSOR);
	if (yych == 's') goto yy27;
	goto yy5;
yy13:
	yych = *++YYCURSOR;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy28;
yy14:
	YYCURSOR = YYMARKER;
	if (yyaccept <= 1) {
		if (yyaccept == 0) {
			goto yy5;
		} else {
			goto yy121;
		}
	} else {
		if (yyaccept == 2) {
			goto yy135;
		} else {
			goto yy139;
		}
	}
yy15:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 2) YYFILL(2);
	yych = *YYCURSOR;
	if (yych == '-') goto yy13;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy15;
	goto yy14;
yy17:
	yych = *++YYCURSOR;
	if (yych == 's') goto yy30;
	goto yy14;
yy18:
	yych = *++YYCURSOR;
	if (yych == 's') goto yy31;
	goto yy14;
yy19:
	yych = *++YYCURSOR;
	if (yych == 'c') goto yy32;
	goto yy14;
yy20:
	yych = *++YYCURSOR;
	if (yych == 'r') goto yy33;
	goto yy14;
yy21:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy34;
	goto yy14;
yy22:
	yych = *++YYCURSOR;
	if (yych == 'o') goto yy31;
	goto yy14;
yy23:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy35;
	goto yy14;
yy24:
	yych = *++YYCURSOR;
	if (yych == 'e') goto yy36;
	goto yy14;
yy25:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '-') goto yy37;
	goto yy25;
yy27:
	yych = *++YYCURSOR;
	if (yych == 't') goto yy39;
	goto yy14;
yy28:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 2) YYFILL(2);
	yych = *YYCURSOR;
	if (yych == '-') goto yy40;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy28;
	goto yy14;
yy30:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy41;
	goto yy14;
yy31:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy42;
	goto yy14;
yy32:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy25;
	goto yy14;
yy33:
	yych = *++YYCURSOR;
	if (yych == 'm') goto yy43;
	goto yy14;
yy34:
	yych = *++YYCURSOR;
	if (yych == 'g') goto yy44;
	if (yych == 'p') goto yy45;
	goto yy14;
yy35:
	yych = *++YYCURSOR;
	if (yych == 'd') goto yy18;
	goto yy14;
yy36:
	yych = *++YYCURSOR;
	if (yych == 'm') goto yy46;
	goto yy14;
yy37:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= ',') {
		if (yych == '\n') goto yy14;
		goto yy25;
	} else {
		if (yych <= '-') goto yy37;
		if (yych <= '/') goto yy25;
		if (yych <= '9') goto yy47;
		goto yy25;
	}
yy39:
	yych = *++YYCURSOR;
	if (yych == 't') goto yy49;
	goto yy14;
yy40:
	yych = *++YYCURSOR;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy50;
	goto yy14;
yy41:
	yych = *++YYCURSOR;
	if (yych == '.') goto yy14;
	if (yych == 'p') goto yy45;
	goto yy53;
yy42:
	yych = *++YYCURSOR;
	if (yych == '.') goto yy14;
	goto yy53;
yy43:
	yych = *++YYCURSOR;
	if (yych == 's') goto yy25;
	goto yy14;
yy44:
	yych = *++YYCURSOR;
	if (yych == 'w') goto yy54;
	goto yy14;
yy45:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy55;
	goto yy14;
yy46:
	yych = *++YYCURSOR;
	if (yych == 'i') goto yy56;
	goto yy14;
yy47:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= ',') {
		if (yych == '\n') goto yy14;
		goto yy25;
	} else {
		if (yych <= '-') goto yy57;
		if (yych <= '/') goto yy25;
		if (yych <= '9') goto yy47;
		goto yy25;
	}
yy49:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy35;
	goto yy14;
yy50:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 2) YYFILL(2);
	yych = *YYCURSOR;
	if (yych == '-') goto yy58;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy50;
	goto yy14;
yy52:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 2) YYFILL(2);
	yych = *YYCURSOR;
yy53:
	if (yych <= '/') {
		if (yych <= ',') goto yy14;
		if (yych <= '-') goto yy52;
		if (yych <= '.') goto yy59;
		goto yy14;
	} else {
		if (yych <= '9') goto yy52;
		if (yych <= '`') goto yy14;
		if (yych <= 'k') goto yy52;
		goto yy14;
	}
yy54:
	yych = *++YYCURSOR;
	if (yych == '4') goto yy60;
	goto yy14;
yy55:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy42;
	goto yy14;
yy56:
	yych = *++YYCURSOR;
	if (yych == 'u') goto yy61;
	goto yy14;
yy57:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= ',') {
		if (yych == '\n') goto yy14;
		goto yy25;
	} else {
		if (yych <= '-') goto yy37;
		if (yych <= '/') goto yy25;
		if (yych <= '9') goto yy62;
		goto yy25;
	}
yy58:
	yych = *++YYCURSOR;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy64;
	goto yy14;
yy59:
	yych = *++YYCURSOR;
	if (yych == '\n') goto yy14;
	goto yy66;
yy60:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy68;
	goto yy14;
yy61:
	yych = *++YYCURSOR;
	if (yych == 'm') goto yy69;
	goto yy14;
yy62:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 2) YYFILL(2);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		if (yych <= ',') goto yy25;
		goto yy57;
	} else {
		if (yych <= '.') goto yy70;
		if (yych <= '/') goto yy25;
		if (yych <= '9') goto yy62;
		goto yy25;
	}
yy64:
	++YYCURSOR;
	if ((YYLIMIT - YYCURSOR) < 8) YYFILL(8);
	yych = *YYCURSOR;
	if (yych == '.') goto yy71;
	if (yych <= '/') goto yy14;
	if (yych <= '9') goto yy64;
	goto yy14;
yy66:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy72;
	goto yy66;
yy68:
	yych = *++YYCURSOR;
	if (yych == 'o') goto yy22;
	goto yy14;
yy69:
	yych = *++YYCURSOR;
	if (yych == 'C') goto yy42;
	goto yy14;
yy70:
	yych = *++YYCURSOR;
	if (yych == '\n') goto yy14;
	goto yy74;
yy71:
	yych = *++YYCURSOR;
	if (yych == 'b') goto yy78;
	if (yych == 'd') goto yy79;
	goto yy77;
yy72:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 'u') goto yy82;
		goto yy66;
	}
yy74:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy83;
	goto yy74;
yy76:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
yy77:
	if (yych == '\n') goto yy14;
	if (yych == 'm') goto yy80;
	goto yy76;
yy78:
	yych = *++YYCURSOR;
	if (yych == 'o') goto yy85;
	goto yy77;
yy79:
	yych = *++YYCURSOR;
	if (yych == 'h') goto yy86;
	goto yy77;
yy80:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= 'l') {
		if (yych == '\n') goto yy14;
		goto yy76;
	} else {
		if (yych <= 'm') goto yy80;
		if (yych == 'o') goto yy87;
		goto yy76;
	}
yy82:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 's') goto yy88;
		goto yy66;
	}
yy83:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'r') goto yy89;
		goto yy74;
	}
yy85:
	yych = *++YYCURSOR;
	if (yych == 't') goto yy90;
	goto yy77;
yy86:
	yych = *++YYCURSOR;
	if (yych == 'c') goto yy91;
	goto yy77;
yy87:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= 'c') {
		if (yych == '\n') goto yy14;
		goto yy76;
	} else {
		if (yych <= 'd') goto yy92;
		if (yych == 'm') goto yy80;
		goto yy76;
	}
yy88:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 'w') goto yy93;
		goto yy66;
	}
yy89:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'e') goto yy94;
		goto yy74;
	}
yy90:
	yych = *++YYCURSOR;
	if (yych == 'h') goto yy95;
	goto yy77;
yy91:
	yych = *++YYCURSOR;
	if (yych == 'p') goto yy96;
	goto yy77;
yy92:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= 'd') {
		if (yych == '\n') goto yy14;
		goto yy76;
	} else {
		if (yych <= 'e') goto yy98;
		if (yych == 'm') goto yy80;
		goto yy76;
	}
yy93:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 'e') goto yy99;
		goto yy66;
	}
yy94:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 's') goto yy100;
		goto yy74;
	}
yy95:
	yych = *++YYCURSOR;
	if (yych == 'e') goto yy101;
	goto yy77;
yy96:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy102;
	goto yy96;
yy98:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == 'm') goto yy96;
	goto yy76;
yy99:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 's') goto yy104;
		goto yy66;
	}
yy100:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'n') goto yy105;
		goto yy74;
	}
yy101:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy106;
	goto yy77;
yy102:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'w') goto yy107;
		goto yy96;
	}
yy104:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 't') goto yy108;
		goto yy66;
	}
yy105:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'e') goto yy109;
		goto yy74;
	}
yy106:
	yych = *++YYCURSOR;
	if (yych == 'l') goto yy96;
	goto yy77;
yy107:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'a') goto yy110;
		goto yy96;
	}
yy108:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy111;
	goto yy66;
yy109:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 't') goto yy112;
		goto yy74;
	}
yy110:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 's') goto yy113;
		goto yy96;
	}
yy111:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '.') {
		if (yych == '\n') goto yy14;
		if (yych <= '-') goto yy66;
		goto yy72;
	} else {
		if (yych <= 'n') {
			if (yych <= 'm') goto yy66;
			goto yy114;
		} else {
			if (yych == 'u') goto yy82;
			goto yy66;
		}
	}
yy112:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy115;
	goto yy74;
yy113:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'h') goto yy116;
		goto yy96;
	}
yy114:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 'e') goto yy117;
		goto yy66;
	}
yy115:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '.') {
		if (yych == '\n') goto yy14;
		if (yych <= '-') goto yy74;
		goto yy83;
	} else {
		if (yych <= 'p') {
			if (yych <= 'o') goto yy74;
			goto yy118;
		} else {
			if (yych == 'r') goto yy89;
			goto yy74;
		}
	}
yy116:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'i') goto yy119;
		goto yy96;
	}
yy117:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy66;
	} else {
		if (yych <= '.') goto yy72;
		if (yych == 't') goto yy120;
		goto yy66;
	}
yy118:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'i') goto yy122;
		goto yy74;
	}
yy119:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'n') goto yy123;
		goto yy96;
	}
yy120:
	yyaccept = 1;
	YYMARKER = ++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy121;
	if (yych == '.') goto yy72;
	goto yy66;
yy121:
#line 18 "bug1479044.s.re"
	{
		return "dsl";
	}
#line 822 "bug1479044.s.c"
yy122:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 't') goto yy124;
		goto yy74;
	}
yy123:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'g') goto yy125;
		goto yy96;
	}
yy124:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 't') goto yy126;
		goto yy74;
	}
yy125:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 't') goto yy127;
		goto yy96;
	}
yy126:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy128;
	goto yy74;
yy127:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'o') goto yy129;
		goto yy96;
	}
yy128:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '.') {
		if (yych == '\n') goto yy14;
		if (yych <= '-') goto yy74;
		goto yy83;
	} else {
		if (yych <= 'e') {
			if (yych <= 'd') goto yy74;
			goto yy130;
		} else {
			if (yych == 'r') goto yy89;
			goto yy74;
		}
	}
yy129:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych == 'n') goto yy131;
		goto yy96;
	}
yy130:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'd') goto yy132;
		goto yy74;
	}
yy131:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy14;
	if (yych == '.') goto yy133;
	goto yy96;
yy132:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy74;
	} else {
		if (yych <= '.') goto yy83;
		if (yych == 'u') goto yy134;
		goto yy74;
	}
yy133:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '.') {
		if (yych == '\n') goto yy14;
		if (yych <= '-') goto yy96;
		goto yy102;
	} else {
		if (yych <= 'e') {
			if (yych <= 'd') goto yy96;
			goto yy136;
		} else {
			if (yych == 'w') goto yy107;
			goto yy96;
		}
	}
yy134:
	yyaccept = 2;
	YYMARKER = ++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy135;
	if (yych == '.') goto yy83;
	goto yy74;
yy135:
#line 23 "bug1479044.s.re"
	{
		return "resnet";
	}
#line 980 "bug1479044.s.c"
yy136:
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych != 'd') goto yy96;
	}
	++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych <= '-') {
		if (yych == '\n') goto yy14;
		goto yy96;
	} else {
		if (yych <= '.') goto yy102;
		if (yych != 'u') goto yy96;
	}
	yyaccept = 3;
	YYMARKER = ++YYCURSOR;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	if (yych == '\n') goto yy139;
	if (yych == '.') goto yy102;
	goto yy96;
yy139:
#line 28 "bug1479044.s.re"
	{
		return "edu";
	}
#line 1014 "bug1479044.s.c"
}
#line 41 "bug1479044.s.re"

}

int main(int argc, char **argv)
{
	int n = 0;
	char *largv[2];

	if (argc < 2)
	{
		argc = 2;
		argv = largv;
		argv[1] = "D-128-208-46-51.dhcp4.washington.edu";
	}
	while(++n < argc)
	{
		char *res = scan281(argv[n]);
		printf("%s\n", res ? res : "<NULL>");
	}
	return 0;
}
