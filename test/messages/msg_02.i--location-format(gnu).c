/* Generated by re2c */
// re2c $INPUT -o $OUTPUT -i --location-format gnu






{
	YYCTYPE yych;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	switch (yych) {
	case 'Z':
	case '[':
	case '\\':
	case ']':
	case '^':
	case '_':
	case '`':
	case 'a':	goto yy4;
	default:	goto yy2;
	}
yy2:
	++YYCURSOR;
	{}
yy4:
	++YYCURSOR;
	{}
}



{
	YYCTYPE yych;
	if (YYLIMIT <= YYCURSOR) YYFILL(1);
	yych = *YYCURSOR;
	switch (yych) {
	case 0x07:
	case 'h':	goto yy9;
	default:	goto yy8;
	}
yy8:
yy9:
	++YYCURSOR;
	{}
}

messages/nested/msg_02.re.inc:2:9: warning: range lower bound (0x61) is greater than upper bound (0x5A), swapping [-Wswapped-range]
messages/nested/msg_02.re.inc:6:11: warning: escape has no effect: '\h' [-Wuseless-escape]
messages/msg_02.i--location-format(gnu).re:11:2: warning: control flow is undefined for strings that match '[\x0-\x6\x8-\x67\x69-\xFF]', use default rule '*' [-Wundefined-control-flow]
