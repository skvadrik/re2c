...
for f in $(find -name '*.re'); do
    ...
    # we need -O1 here to have tailcall optimization
    # (otherwise some test run into stack overflow issues)
    g++ -O1 -fsanitize=leak -fsanitize=address -g -Wall -oexample  "$ctest" \
        && ./example 2>/dev/null || { echo "*** error ***"; exit 1; }

    rm -f example example.o "$ctest"
    cd $root_dir
done
