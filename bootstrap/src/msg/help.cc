...
"              fixed  distance  to some other tag (called \"base\"). In such cases only tha base tag\n"
"              needs to be tracked, and the value of the fixed tag can be computed as the value of\n"
"              the  base  tag plus a static offset. For tags that are under alternative or repeti‐\n"
"              tion it is also necessary to check if the base tag has a no-match  value  (in  that\n"
"              case  fixed  tag should also be set to no-match, disregarding the offset). For tags\n"
"              in top-level concatenation the check is not needed, because they always match.\n"
"\n"
"WARNINGS\n"
