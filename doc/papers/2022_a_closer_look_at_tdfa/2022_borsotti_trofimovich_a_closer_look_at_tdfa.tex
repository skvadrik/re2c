...
\begin{multicols}{2}
    ...
    \ElseIf {$e = e_1 \mid e_2$} {
        ...
        $(\Sigma, T_1, Q_1, q_1, q'_2, \Delta_1) = \retonfa (e_1, q'_2)$ \;
...
\nonl\Fn {$\underline{transition \Xund regops \big( C, R, V \big)} \smallskip$} {
    ...
    \For {each $(q, r, h, l) \in C$} {
        \For {each tag $t \in T$} {
            \If {$h_t = history(h, t) \neq \epsilon$ and \;
                    \quad\quad $(history(l, t) = \epsilon$ or $t$ is a multi-tag$)$} {
                ...
                \If {operation $i \leftarrow v$ is not in $O$} {
...
\nonl\Fn {$\underline{final \Xund regops \big( R_f, r, l \big)} \smallskip$} {
    ...
    \For {each tag $t \in T$} {
        \If {$l_t = history(l, t) \neq \epsilon$} {
            ...
            append $R_f[t] \leftarrow r[t]$ to $O$ \;
        }
    }
    \Return $O$ \;
}
