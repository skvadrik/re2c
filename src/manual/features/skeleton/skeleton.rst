...
an ``.input`` file with strings derived from the DFA and a ``.keys`` file with expected match results.
...
of each disjoint character range in the DFA (see `Generating data`_ section for details).
