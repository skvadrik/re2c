...
With the ``-D, --emit-dot`` option, re2c does not generate C/C++ code.
Instead, it dumps the generated DFA in the `DOT format <https://en.wikipedia.org/wiki/DOT_%28graph_description_language%29>`_.
One can convert this dump to an image of the DFA using `graphviz <http://www.graphviz.org/>`_ or another library.
...
Render it with ```dot -Gratio=0.3 -Tpng -o php_json_dot.png php_json.dot```:

.. image:: php_json_dot.png
    :width: 100%

Render it with ```neato -Elen=4 -Tpng -o php_json_neato.png php_json.dot```:

.. image:: php_json_neato.png
    :width: 70%

The generated graph is sometimes very large and requires careful tuning of the rendering parameters.
