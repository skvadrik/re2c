...
First and foremost, :ref:`end of input rule <manual/manual_c:sentinel with bounds checks>`
...
:ref:`number of ways <manual/manual_c:handling the end of input>` to handle
EOF situation. The simplest and the most efficient method is
:ref:`using a sentinel symbol <manual/manual_c:sentinel>`, but it
...
by :ref:`using the generic API <manual/manual_c:custom checks>`. However,
