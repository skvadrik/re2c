...
class GenTargetElem : public RenderCallback {
    std::ostream& os;
  ...
  public:
    GenTargetElem(std::ostream& os, const std::string& prefix, const std::string& base,
            const std::string& label)
        : os(os), prefix(prefix), base(base), label(label) {}
    GenTargetElem(std::ostream& os, const std::string& prefix, const uint32_t base,
            const uint32_t label)
        : GenTargetElem(os, prefix, std::to_string(base), std::to_string(label)) {}
    ...
    FORBID_COPY(GenTargetElem);
};

class GenTargetGoto : public RenderCallback {
    std::ostream& os;
    const opt_t* opts;
    const std::string& prefix;
    const std::string& base;
  ...
  public:
    GenTargetGoto(std::ostream& os, const opt_t* opts, const std::string& prefix, const std::string& base,
            const std::string& elem)
        : os(os), opts(opts), prefix(prefix), base(base), index(elem) {}
    GenTargetGoto(std::ostream& os, const opt_t* opts, const std::string& prefix, const uint32_t base,
            const std::string& elem)
        : GenTargetGoto(os, opts, prefix, std::to_string(base), elem) {}
...
static CodeList* gen_gocp_table(Output& output, const CodeGoCpTable* go, uint32_t min_index) {
    ...
    for (uint32_t i = 0; i < CodeGoCpTable::TABLE_SIZE; ++i) {
        GenTargetElem callback(buf.stream(), opts->label_prefix, min_index, go->table[i]->label->index);
...
static CodeList* gen_gocp(Output& output, const Adfa& dfa, const CodeGoCp* go, const State* from) {
    ...
    GenTargetGoto callback(buf.stream(), opts, opts->label_prefix, min_index, opts->var_char);
...
static void emit_accept(
    ...
    if (opts->computed_gotos && nacc >= opts->computed_gotos_threshold && !have_tags) {
        ...
        for (uint32_t i = 0; i < nacc; ++i) {
            GenTargetElem callback(buf.stream(), opts->label_prefix, min_index,
        ...
        GenTargetGoto callback(buf.stream(), opts, opts->label_prefix, min_index, var);
...
static CodeList* gen_cond_goto(Output& output) {
    ...
    if (opts->computed_gotos) {
        GenTargetGoto callback(buf.stream(), opts, opts->cond_label_prefix, conds.front().name, getcond);
...
static CodeList* gen_cond_table(Output& output) {
    ...
    for (uint32_t i = 0; i < conds.size(); ++i) {
        GenTargetElem callback(buf.stream(), opts->cond_label_prefix, conds.front().name,
                conds[i].name);
