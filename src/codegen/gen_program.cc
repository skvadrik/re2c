...
OutputBlock::OutputBlock(InputBlock kind, const std::string &name,
    const loc_t &loc)
    : kind(kind)
    , name(name)
    , loc(loc)
...
void Output::new_block(Opt &opts, InputBlock kind, const std::string &name,
    const loc_t &loc)
{
    ...
    if (kind == InputBlock::USE) {
        ...
        block->name = "#line_" + to_string(loc.line) + "_block_use_" + name;
    } else if (name.empty()) {
        ...
        block->name = "#line_" + to_string(loc.line) + "_block_" + to_string(pblocks->size());
    }
    ...
    for (const OutputBlock *b : *pblocks) {
        if (b->name == block->name) {
            msg.error(loc, "block named '%s' is already defined on line %u",
                block->name.c_str(), b->loc.line);
            exit(1);
        }
    }

    block->opts = opts.snapshot();
