...
static inline bool init_input(Input *in, const char *fname) {
...
#define RAGEL_BENCH() \
void bench::operator()(benchmark::State &state, const char* input, long expected) const { \
    long count; \
    const char *error = nullptr; \
    for (auto _ : state) { \
        Input in; \
        if (!init_input(&in, input)) { \
    ...
    if (count != expected) state.SkipWithError("wrong count"); \
}

#define RAGEL_TEST() \
bool test(const char* input, long expected) { \
    long count; \
    const char *error = nullptr; \
    Input in; \
    if (!init_input(&in, input)) { \
