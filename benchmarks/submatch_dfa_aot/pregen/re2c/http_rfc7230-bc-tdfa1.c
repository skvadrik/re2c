/* Generated by re2c 2.0.3 */
#line 1 "../../../benchmarks/submatch_dfa_aot/src/re2c/http_rfc7230.re"
#include <assert.h>
#include <stdlib.h>
#include "common.h"

#line 15 "../../../benchmarks/submatch_dfa_aot/src/re2c/common.re"


typedef struct taglist_t {
    struct taglist_t *pred;
    long dist;
} taglist_t;

typedef struct taglistpool_t {
    taglist_t *head;
    taglist_t *next;
    taglist_t *last;
} taglistpool_t;

typedef struct {
    char *buf;
    char *lim;
    char *cur;
    char *mar;
    char *tok;
    char *yyt1;
char *yyt10;
char *yyt11;
char *yyt12;
char *yyt13;
char *yyt14;
char *yyt15;
char *yyt16;
char *yyt17;
char *yyt18;
char *yyt19;
char *yyt2;
char *yyt20;
char *yyt21;
char *yyt22;
char *yyt23;
char *yyt24;
char *yyt25;
char *yyt26;
char *yyt27;
char *yyt28;
char *yyt29;
char *yyt3;
char *yyt30;
char *yyt4;
char *yyt5;
char *yyt6;
char *yyt7;
char *yyt8;
char *yyt9;

    taglist_t *yyt31;
taglist_t *yyt32;
taglist_t *yyt33;
taglist_t *yyt34;
taglist_t *yyt35;

    taglistpool_t tlp;
    int eof;
} input_t;

static inline void taglistpool_clear(taglistpool_t *tlp, input_t *in)
{
    tlp->next = tlp->head;
    in->yyt31 = 0;
in->yyt32 = 0;
in->yyt33 = 0;
in->yyt34 = 0;
in->yyt35 = 0;

}

static inline void taglistpool_init(taglistpool_t *tlp)
{
    static const unsigned size = 1024 * 1024;
    tlp->head = (taglist_t*)malloc(size * sizeof(taglist_t));
    tlp->next = tlp->head;
    tlp->last = tlp->head + size;
}

static inline void taglistpool_free(taglistpool_t *tlp)
{
    free(tlp->head);
    tlp->head = tlp->next = tlp->last = NULL;
}

static inline void taglist(taglist_t **ptl, const char *b, const char *t, taglistpool_t *tlp)
{
#ifdef GROW_MTAG_LIST
    if (tlp->next >= tlp->last) {
        const unsigned size = tlp->last - tlp->head;
        taglist_t *head = (taglist_t*)malloc(2 * size * sizeof(taglist_t));
        memcpy(head, tlp->head, size * sizeof(taglist_t));
        free(tlp->head);
        tlp->head = head;
        tlp->next = head + size;
        tlp->last = head + size * 2;
    }
#else
    assert(tlp->next < tlp->last);
#endif
    taglist_t *tl = tlp->next++;
    tl->pred = *ptl;
    tl->dist = t - b;
    *ptl = tl;
}

#line 4 "../../../benchmarks/submatch_dfa_aot/src/re2c/include_bc/fill.re"


#define YYMAXFILL 66


static inline int fill(input_t *in, size_t need)
{
    size_t free;
    if (in->eof) return 1;

    free = in->tok - in->buf;
    assert(free >= need);

    memmove(in->buf, in->tok, in->lim - in->tok);
    in->lim -= free;
    in->cur -= free;
    in->mar -= free;
    in->tok -= free;
    if (in->yyt1) in->yyt1 -= free;
if (in->yyt10) in->yyt10 -= free;
if (in->yyt11) in->yyt11 -= free;
if (in->yyt12) in->yyt12 -= free;
if (in->yyt13) in->yyt13 -= free;
if (in->yyt14) in->yyt14 -= free;
if (in->yyt15) in->yyt15 -= free;
if (in->yyt16) in->yyt16 -= free;
if (in->yyt17) in->yyt17 -= free;
if (in->yyt18) in->yyt18 -= free;
if (in->yyt19) in->yyt19 -= free;
if (in->yyt2) in->yyt2 -= free;
if (in->yyt20) in->yyt20 -= free;
if (in->yyt21) in->yyt21 -= free;
if (in->yyt22) in->yyt22 -= free;
if (in->yyt23) in->yyt23 -= free;
if (in->yyt24) in->yyt24 -= free;
if (in->yyt25) in->yyt25 -= free;
if (in->yyt26) in->yyt26 -= free;
if (in->yyt27) in->yyt27 -= free;
if (in->yyt28) in->yyt28 -= free;
if (in->yyt29) in->yyt29 -= free;
if (in->yyt3) in->yyt3 -= free;
if (in->yyt30) in->yyt30 -= free;
if (in->yyt4) in->yyt4 -= free;
if (in->yyt5) in->yyt5 -= free;
if (in->yyt6) in->yyt6 -= free;
if (in->yyt7) in->yyt7 -= free;
if (in->yyt8) in->yyt8 -= free;
if (in->yyt9) in->yyt9 -= free;


    in->lim += fread(in->lim, 1, free, stdin);

    if (in->lim < in->buf + SIZE) {
        in->eof = 1;
        memset(in->lim, 0, YYMAXFILL);
        in->lim += YYMAXFILL;
    }

    return 0;
}

static inline void init_input(input_t *in)
{
    in->buf = (char*) malloc(SIZE + YYMAXFILL);
    in->lim = in->buf + SIZE;
    in->cur = in->lim;
    in->mar = in->lim;
    in->tok = in->lim;
    in->yyt1 = 0;
in->yyt10 = 0;
in->yyt11 = 0;
in->yyt12 = 0;
in->yyt13 = 0;
in->yyt14 = 0;
in->yyt15 = 0;
in->yyt16 = 0;
in->yyt17 = 0;
in->yyt18 = 0;
in->yyt19 = 0;
in->yyt2 = 0;
in->yyt20 = 0;
in->yyt21 = 0;
in->yyt22 = 0;
in->yyt23 = 0;
in->yyt24 = 0;
in->yyt25 = 0;
in->yyt26 = 0;
in->yyt27 = 0;
in->yyt28 = 0;
in->yyt29 = 0;
in->yyt3 = 0;
in->yyt30 = 0;
in->yyt4 = 0;
in->yyt5 = 0;
in->yyt6 = 0;
in->yyt7 = 0;
in->yyt8 = 0;
in->yyt9 = 0;

    in->yyt31 = 0;
in->yyt32 = 0;
in->yyt33 = 0;
in->yyt34 = 0;
in->yyt35 = 0;

    taglistpool_init(&in->tlp);
    in->eof = 0;
}


static inline void free_input(input_t *in)
{
    free(in->buf);
    taglistpool_free(&in->tlp);
}

static int lex(input_t *in, Output *out);

int main(int argc, char **argv)
{
    PRE;
    input_t in;
    Output out;

    init_input(&in);
    init_output(&out);

    switch (lex(&in, &out)) {
        case 0:  break;
        case 1:  fprintf(stderr, "*** %s: syntax error\n", argv[0]); break;
        case 2:  fprintf(stderr, "*** %s: yyfill error\n", argv[0]); break;
        default: fprintf(stderr, "*** %s: panic\n", argv[0]); break;
    }

    flush(&out);
    free_output(&out);
    free_input(&in);

    POST;
    return 0;
}


static void print_headers(Output *out, const char *tok,
    const taglist_t *h1, const taglist_t *h2, const taglist_t *h3, const taglist_t *h4)
{
    if (!h4) return;
    print_headers(out, tok, h1->pred, h2->pred, h3->pred, h4->pred);
    OUT("header: ", tok + h1->dist, tok + h2->dist);
    OUT("value: ", tok + h3->dist, tok + h4->dist);
}

static int lex(input_t *in, Output *out)
{
    const char *of, *au, *at,
        *hs1, *hs3, *m1, *p1, *p3, *p5, *q1, *q3,
        *hs2, *hs4, *m2, *p2, *p4, *p6, *q2, *q4,
        *r1, *r3, *rp1, *s1, *st1, *u1, *u3, *v1, *v3,
        *r2, *r4, *rp2, *s2, *st2, *u2, *u4, *v2, *v4;
    taglist_t *h1, *h2, *h3, *h4;

loop:
    in->tok = in->cur;

#line 280 "gen/re2c/http_rfc7230-bc-tdfa1.c"
{
	char yych;
	if ((in->lim - in->cur) < 66) if (fill(in, 66) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:	goto yy2;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':
		in->yyt1 = in->cur;
		goto yy6;
	case 'H':
		in->yyt1 = in->cur;
		goto yy7;
	default:	goto yy4;
	}
yy2:
	++in->cur;
#line 3 "../../../benchmarks/submatch_dfa_aot/src/re2c/include_bc/fill.re"
	{ return 0; }
#line 374 "gen/re2c/http_rfc7230-bc-tdfa1.c"
yy4:
	++in->cur;
yy5:
#line 97 "../../../benchmarks/submatch_dfa_aot/src/re2c/http_rfc7230.re"
	{ return 1; }
#line 380 "gen/re2c/http_rfc7230-bc-tdfa1.c"
yy6:
	yych = *(in->mar = ++in->cur);
	switch (yych) {
	case ' ':
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':	goto yy11;
	default:	goto yy5;
	}
yy7:
	yych = *(in->mar = ++in->cur);
	switch (yych) {
	case ' ':
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':	goto yy11;
	case 'T':	goto yy12;
	default:	goto yy5;
	}
yy8:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt3 = in->yyt10 = in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case '_':
	case '~':
		in->yyt4 = in->yyt9 = NULL;
		in->yyt3 = in->yyt5 = in->yyt10 = in->cur;
		goto yy14;
	case '%':
		in->yyt4 = in->yyt9 = NULL;
		in->yyt3 = in->yyt5 = in->yyt10 = in->cur;
		goto yy16;
	case '/':
		in->yyt26 = in->cur;
		goto yy17;
	case ':':
		in->yyt4 = in->yyt9 = NULL;
		in->yyt3 = in->yyt5 = in->yyt10 = in->yyt11 = in->cur;
		goto yy19;
	case '?':
		in->yyt26 = in->yyt27 = in->cur;
		goto yy20;
	case '@':
		in->yyt3 = in->yyt5 = in->yyt9 = in->cur;
		goto yy21;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
		in->yyt4 = in->yyt9 = NULL;
		in->yyt3 = in->yyt5 = in->yyt10 = in->yyt13 = in->cur;
		goto yy22;
	case '[':
		in->yyt4 = in->yyt9 = NULL;
		in->yyt3 = in->yyt10 = in->cur;
		goto yy24;
	default:	goto yy9;
	}
yy9:
	in->cur = in->mar;
	goto yy5;
yy10:
	++in->cur;
	if ((in->lim - in->cur) < 62) if (fill(in, 62) != 0) return 1;
	yych = *in->cur;
yy11:
	switch (yych) {
	case ' ':
		in->yyt2 = in->cur;
		goto yy8;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':	goto yy10;
	default:	goto yy9;
	}
yy12:
	yych = *++in->cur;
	switch (yych) {
	case 'T':	goto yy25;
	default:	goto yy11;
	}
yy13:
	yych = *++in->cur;
	switch (yych) {
	case 'H':	goto yy26;
	default:	goto yy9;
	}
yy14:
	++in->cur;
	if ((in->lim - in->cur) < 61) if (fill(in, 61) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy14;
	case '%':	goto yy16;
	case ':':
		in->yyt11 = in->cur;
		goto yy19;
	case '@':
		in->yyt9 = in->cur;
		goto yy21;
	default:	goto yy9;
	}
yy16:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy27;
	default:	goto yy9;
	}
yy17:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt28 = NULL;
		in->yyt27 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy17;
	case '%':	goto yy28;
	case '?':
		in->yyt27 = in->cur;
		goto yy20;
	default:	goto yy9;
	}
yy19:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->yyt12 = in->cur;
		goto yy13;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt12 = in->cur;
		goto yy32;
	default:	goto yy30;
	}
yy20:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = NULL;
		in->yyt7 = in->yyt28 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '?':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt28 = in->cur;
		goto yy34;
	case '%':
		in->yyt28 = in->cur;
		goto yy36;
	default:	goto yy9;
	}
yy21:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt10 = in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt10 = in->cur;
		goto yy37;
	case '%':
		in->yyt10 = in->cur;
		goto yy39;
	case ':':
		in->yyt10 = in->yyt11 = in->cur;
		goto yy40;
	case '[':
		in->yyt4 = in->yyt5;
		in->yyt10 = in->cur;
		goto yy24;
	default:	goto yy9;
	}
yy22:
	++in->cur;
	if ((in->lim - in->cur) < 65) if (fill(in, 65) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case ',':
	case ';':
	case '=':
	case '_':
	case '~':	goto yy14;
	case '%':	goto yy16;
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':	goto yy22;
	case ':':
		in->yyt11 = in->yyt14 = in->cur;
		goto yy41;
	case '@':
		in->yyt9 = in->cur;
		goto yy21;
	default:	goto yy9;
	}
yy24:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy42;
	case ':':	goto yy43;
	case 'v':	goto yy44;
	default:	goto yy9;
	}
yy25:
	yych = *++in->cur;
	switch (yych) {
	case 'P':	goto yy45;
	default:	goto yy11;
	}
yy26:
	yych = *++in->cur;
	switch (yych) {
	case 'T':	goto yy46;
	default:	goto yy9;
	}
yy27:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy14;
	default:	goto yy9;
	}
yy28:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy47;
	default:	goto yy9;
	}
yy29:
	++in->cur;
	if ((in->lim - in->cur) < 60) if (fill(in, 60) != 0) return 1;
	yych = *in->cur;
yy30:
	switch (yych) {
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy29;
	case '%':	goto yy31;
	case '@':
		in->yyt9 = in->cur;
		goto yy21;
	default:	goto yy9;
	}
yy31:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy48;
	default:	goto yy9;
	}
yy32:
	++in->cur;
	if ((in->lim - in->cur) < 60) if (fill(in, 60) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy29;
	case '%':	goto yy31;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy32;
	case '@':
		in->yyt9 = in->cur;
		goto yy21;
	default:	goto yy9;
	}
yy34:
	++in->cur;
	if ((in->lim - in->cur) < 11) if (fill(in, 11) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = NULL;
		in->yyt7 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '?':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy34;
	case '%':	goto yy36;
	default:	goto yy9;
	}
yy36:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy49;
	default:	goto yy9;
	}
yy37:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy37;
	case '%':	goto yy39;
	case ':':
		in->yyt11 = in->cur;
		goto yy40;
	default:	goto yy9;
	}
yy39:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy50;
	default:	goto yy9;
	}
yy40:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->yyt12 = in->cur;
		goto yy13;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt12 = in->cur;
		goto yy51;
	default:	goto yy9;
	}
yy41:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->yyt12 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt23 = in->cur;
		goto yy53;
	case '%':
		in->yyt23 = in->cur;
		goto yy55;
	case '/':
		in->yyt23 = in->cur;
		goto yy56;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt12 = in->yyt23 = in->cur;
		goto yy57;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt23 = in->yyt24 = in->cur;
		goto yy59;
	case '@':
		in->yyt9 = in->yyt23 = in->cur;
		goto yy60;
	default:	goto yy9;
	}
yy42:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy61;
	case ':':	goto yy62;
	default:	goto yy9;
	}
yy43:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy63;
	default:	goto yy9;
	}
yy44:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy9;
	default:	goto yy65;
	}
yy45:
	yych = *++in->cur;
	switch (yych) {
	case '/':	goto yy66;
	default:	goto yy11;
	}
yy46:
	yych = *++in->cur;
	switch (yych) {
	case 'T':	goto yy67;
	default:	goto yy9;
	}
yy47:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy17;
	default:	goto yy9;
	}
yy48:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy29;
	default:	goto yy9;
	}
yy49:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy34;
	default:	goto yy9;
	}
yy50:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy37;
	default:	goto yy9;
	}
yy51:
	++in->cur;
	if ((in->lim - in->cur) < 11) if (fill(in, 11) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->cur;
		goto yy13;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy51;
	default:	goto yy9;
	}
yy53:
	++in->cur;
	if ((in->lim - in->cur) < 60) if (fill(in, 60) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt24 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy53;
	case '%':	goto yy55;
	case '/':	goto yy68;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt24 = in->cur;
		goto yy59;
	case '@':
		in->yyt9 = in->cur;
		goto yy60;
	default:	goto yy9;
	}
yy55:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy70;
	default:	goto yy9;
	}
yy56:
	yych = *++in->cur;
	switch (yych) {
	case '/':	goto yy72;
	default:	goto yy69;
	}
yy57:
	++in->cur;
	if ((in->lim - in->cur) < 60) if (fill(in, 60) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy53;
	case '%':	goto yy55;
	case '/':	goto yy68;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy57;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt24 = in->cur;
		goto yy59;
	case '@':
		in->yyt9 = in->cur;
		goto yy60;
	default:	goto yy9;
	}
yy59:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt6 = in->yyt25 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '?':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt25 = in->cur;
		goto yy73;
	case '%':
		in->yyt25 = in->cur;
		goto yy75;
	default:	goto yy9;
	}
yy60:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt10 = in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt10 = in->cur;
		goto yy76;
	case '%':
		in->yyt10 = in->cur;
		goto yy78;
	case ':':
		in->yyt10 = in->yyt11 = in->cur;
		goto yy79;
	case '[':
		in->yyt4 = in->yyt5;
		in->yyt10 = in->cur;
		goto yy24;
	default:	goto yy77;
	}
yy61:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy80;
	case ':':	goto yy62;
	default:	goto yy9;
	}
yy62:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy81;
	case ':':	goto yy82;
	default:	goto yy9;
	}
yy63:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy83;
	case '1':	goto yy84;
	case '2':	goto yy85;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy86;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy87;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy64:
	++in->cur;
	if ((in->lim - in->cur) < 2) if (fill(in, 2) != 0) return 1;
	yych = *in->cur;
yy65:
	switch (yych) {
	case '.':	goto yy89;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy64;
	default:	goto yy9;
	}
yy66:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy90;
	default:	goto yy9;
	}
yy67:
	yych = *++in->cur;
	switch (yych) {
	case 'P':	goto yy91;
	default:	goto yy9;
	}
yy68:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
yy69:
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt24 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy68;
	case '%':	goto yy71;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt24 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy70:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy53;
	default:	goto yy9;
	}
yy71:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy92;
	default:	goto yy9;
	}
yy72:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt15 = in->yyt16 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt15 = in->yyt16 = NULL;
		in->yyt3 = in->yyt17 = in->cur;
		goto yy93;
	case '%':
		in->yyt15 = in->yyt16 = NULL;
		in->yyt3 = in->yyt17 = in->cur;
		goto yy95;
	case '/':
		in->yyt15 = in->yyt16 = in->yyt19 = in->yyt20 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->cur;
		goto yy96;
	case ':':
		in->yyt15 = in->yyt16 = NULL;
		in->yyt3 = in->yyt17 = in->yyt18 = in->cur;
		goto yy98;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	case '@':
		in->yyt3 = in->yyt16 = in->cur;
		goto yy99;
	case '[':
		in->yyt15 = in->yyt16 = NULL;
		in->yyt17 = in->cur;
		goto yy100;
	default:	goto yy9;
	}
yy73:
	++in->cur;
	if ((in->lim - in->cur) < 11) if (fill(in, 11) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt6 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '?':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy73;
	case '%':	goto yy75;
	default:	goto yy9;
	}
yy75:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy101;
	default:	goto yy9;
	}
yy76:
	++in->cur;
	if ((in->lim - in->cur) < 13) if (fill(in, 13) != 0) return 1;
	yych = *in->cur;
yy77:
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt11 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy76;
	case '%':	goto yy78;
	case '/':
	case '@':	goto yy68;
	case ':':
		in->yyt11 = in->cur;
		goto yy79;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt24 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy78:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy102;
	default:	goto yy9;
	}
yy79:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->yyt12 = in->cur;
		goto yy13;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt12 = in->cur;
		goto yy103;
	default:	goto yy69;
	}
yy80:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy105;
	case ':':	goto yy62;
	default:	goto yy9;
	}
yy81:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy106;
	case ':':	goto yy107;
	default:	goto yy9;
	}
yy82:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy108;
	case '1':	goto yy109;
	case '2':	goto yy110;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy111;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy112;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy83:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy114;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy84:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy116;
	case ':':	goto yy115;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy114;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy85:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy116;
	case '5':	goto yy117;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy118;
	case ':':	goto yy115;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy114;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy86:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy118;
	case ':':	goto yy115;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy114;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy87:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy114;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy88:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt5 = in->yyt6 = in->yyt7 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt11 = in->cur;
		goto yy13;
	case ':':
		in->yyt5 = in->yyt4;
		in->yyt11 = in->cur;
		goto yy40;
	default:	goto yy9;
	}
yy89:
	yych = *++in->cur;
	switch (yych) {
	case ']':	goto yy9;
	default:	goto yy120;
	}
yy90:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy121;
	default:	goto yy9;
	}
yy91:
	yych = *++in->cur;
	switch (yych) {
	case '/':	goto yy122;
	default:	goto yy9;
	}
yy92:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy68;
	default:	goto yy9;
	}
yy93:
	++in->cur;
	if ((in->lim - in->cur) < 62) if (fill(in, 62) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy93;
	case '%':	goto yy95;
	case '/':
		in->yyt19 = in->yyt20 = NULL;
		in->yyt18 = in->yyt21 = in->cur;
		goto yy96;
	case ':':
		in->yyt18 = in->cur;
		goto yy98;
	case '?':
		in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	case '@':
		in->yyt16 = in->cur;
		goto yy99;
	default:	goto yy9;
	}
yy95:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy123;
	default:	goto yy9;
	}
yy96:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy96;
	case '%':	goto yy124;
	case '?':
		in->yyt23 = in->yyt24 = NULL;
		in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy98:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '/':
		in->yyt19 = in->yyt20 = in->yyt21 = in->cur;
		goto yy96;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt19 = in->cur;
		goto yy128;
	case '?':
		in->yyt23 = in->yyt24 = NULL;
		in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy126;
	}
yy99:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt15 = in->yyt3;
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':
		in->yyt17 = in->cur;
		goto yy130;
	case '%':
		in->yyt17 = in->cur;
		goto yy132;
	case '/':
		in->yyt15 = in->yyt3;
		in->yyt19 = in->yyt20 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->cur;
		goto yy96;
	case ':':
		in->yyt17 = in->yyt18 = in->cur;
		goto yy133;
	case '?':
		in->yyt15 = in->yyt3;
		in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = NULL;
		in->yyt17 = in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	case '[':
		in->yyt15 = in->yyt3;
		in->yyt17 = in->cur;
		goto yy100;
	default:	goto yy9;
	}
yy100:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy134;
	case ':':	goto yy135;
	case 'v':	goto yy136;
	default:	goto yy9;
	}
yy101:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy73;
	default:	goto yy9;
	}
yy102:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy76;
	default:	goto yy9;
	}
yy103:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt4 = in->yyt5;
		in->yyt6 = in->yyt7 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt5 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '/':
	case ':':
	case ';':
	case '=':
	case '@':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy68;
	case '%':	goto yy71;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy103;
	case '?':
		in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = NULL;
		in->yyt24 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy105:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy62;
	default:	goto yy9;
	}
yy106:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy137;
	case ':':	goto yy107;
	default:	goto yy9;
	}
yy107:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy138;
	case ':':	goto yy139;
	default:	goto yy9;
	}
yy108:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy140;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy109:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy142;
	case ':':	goto yy141;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy140;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy110:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy142;
	case '5':	goto yy143;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy144;
	case ':':	goto yy141;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy140;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy111:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy144;
	case ':':	goto yy141;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy140;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy112:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy140;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy113:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy145;
	case '1':	goto yy146;
	case '2':	goto yy147;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy148;
	default:	goto yy9;
	}
yy114:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy149;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy115:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy108;
	case '1':	goto yy109;
	case '2':	goto yy110;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy111;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy112;
	default:	goto yy9;
	}
yy116:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy150;
	case ':':	goto yy115;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy149;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy117:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy150;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy149;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy118:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy149;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy119:
	++in->cur;
	if ((in->lim - in->cur) < 13) if (fill(in, 13) != 0) return 1;
	yych = *in->cur;
yy120:
	switch (yych) {
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy119;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy121:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy151;
	default:	goto yy9;
	}
yy122:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy152;
	default:	goto yy9;
	}
yy123:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy93;
	default:	goto yy9;
	}
yy124:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy153;
	default:	goto yy9;
	}
yy125:
	++in->cur;
	if ((in->lim - in->cur) < 61) if (fill(in, 61) != 0) return 1;
	yych = *in->cur;
yy126:
	switch (yych) {
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy125;
	case '%':	goto yy127;
	case '@':
		in->yyt16 = in->cur;
		goto yy99;
	default:	goto yy9;
	}
yy127:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy154;
	default:	goto yy9;
	}
yy128:
	++in->cur;
	if ((in->lim - in->cur) < 61) if (fill(in, 61) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy125;
	case '%':	goto yy127;
	case '/':
		in->yyt20 = in->yyt21 = in->cur;
		goto yy96;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy128;
	case '?':
		in->yyt23 = in->yyt24 = NULL;
		in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	case '@':
		in->yyt16 = in->cur;
		goto yy99;
	default:	goto yy9;
	}
yy130:
	++in->cur;
	if ((in->lim - in->cur) < 13) if (fill(in, 13) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt15 = in->yyt3;
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy130;
	case '%':	goto yy132;
	case '/':
		in->yyt15 = in->yyt3;
		in->yyt19 = in->yyt20 = NULL;
		in->yyt18 = in->yyt21 = in->cur;
		goto yy96;
	case ':':
		in->yyt18 = in->cur;
		goto yy133;
	case '?':
		in->yyt15 = in->yyt3;
		in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy132:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy155;
	default:	goto yy9;
	}
yy133:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt15 = in->yyt3;
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '/':
		in->yyt15 = in->yyt3;
		in->yyt19 = in->yyt20 = in->yyt21 = in->cur;
		goto yy96;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
		in->yyt19 = in->cur;
		goto yy156;
	case '?':
		in->yyt15 = in->yyt3;
		in->yyt23 = in->yyt24 = NULL;
		in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy134:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy158;
	case ':':	goto yy159;
	default:	goto yy9;
	}
yy135:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy160;
	default:	goto yy9;
	}
yy136:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy9;
	default:	goto yy162;
	}
yy137:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy163;
	case ':':	goto yy107;
	default:	goto yy9;
	}
yy138:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy164;
	case ':':	goto yy165;
	default:	goto yy9;
	}
yy139:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy166;
	case '1':	goto yy167;
	case '2':	goto yy168;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy169;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy170;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy140:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy171;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy141:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy166;
	case '1':	goto yy167;
	case '2':	goto yy168;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy169;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy170;
	default:	goto yy9;
	}
yy142:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy172;
	case ':':	goto yy141;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy171;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy143:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy172;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy171;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy144:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy171;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy145:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy173;
	default:	goto yy9;
	}
yy146:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy173;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy148;
	default:	goto yy9;
	}
yy147:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy173;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy148;
	case '5':	goto yy174;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy145;
	default:	goto yy9;
	}
yy148:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy173;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy145;
	default:	goto yy9;
	}
yy149:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy175;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy150:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy175;
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy151:
	yych = *++in->cur;
	switch (yych) {
	case ' ':	goto yy176;
	default:	goto yy9;
	}
yy152:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy177;
	default:	goto yy9;
	}
yy153:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy96;
	default:	goto yy9;
	}
yy154:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy125;
	default:	goto yy9;
	}
yy155:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy130;
	default:	goto yy9;
	}
yy156:
	++in->cur;
	if ((in->lim - in->cur) < 12) if (fill(in, 12) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case ' ':
		in->yyt15 = in->yyt3;
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '/':
		in->yyt15 = in->yyt3;
		in->yyt20 = in->yyt21 = in->cur;
		goto yy96;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy156;
	case '?':
		in->yyt15 = in->yyt3;
		in->yyt23 = in->yyt24 = NULL;
		in->yyt20 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy158:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy178;
	case ':':	goto yy159;
	default:	goto yy9;
	}
yy159:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy179;
	case ':':	goto yy180;
	default:	goto yy9;
	}
yy160:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy181;
	case '1':	goto yy182;
	case '2':	goto yy183;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy184;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy185;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy161:
	++in->cur;
	if ((in->lim - in->cur) < 2) if (fill(in, 2) != 0) return 1;
	yych = *in->cur;
yy162:
	switch (yych) {
	case '.':	goto yy187;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy161;
	default:	goto yy9;
	}
yy163:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy107;
	default:	goto yy9;
	}
yy164:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy188;
	case ':':	goto yy165;
	default:	goto yy9;
	}
yy165:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy189;
	case ':':	goto yy190;
	default:	goto yy9;
	}
yy166:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy191;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy167:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy193;
	case ':':	goto yy192;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy191;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy168:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy193;
	case '5':	goto yy194;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy195;
	case ':':	goto yy192;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy191;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy169:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy195;
	case ':':	goto yy192;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy191;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy170:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy191;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy171:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy196;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy172:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy196;
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy173:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy197;
	case '1':	goto yy198;
	case '2':	goto yy199;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy200;
	default:	goto yy9;
	}
yy174:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy173;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy145;
	default:	goto yy9;
	}
yy175:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy115;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy176:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy201;
	default:	goto yy9;
	}
yy177:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy202;
	default:	goto yy9;
	}
yy178:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy203;
	case ':':	goto yy159;
	default:	goto yy9;
	}
yy179:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy204;
	case ':':	goto yy205;
	default:	goto yy9;
	}
yy180:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy206;
	case '1':	goto yy207;
	case '2':	goto yy208;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy209;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy210;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy181:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy212;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy182:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy214;
	case ':':	goto yy213;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy212;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy183:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy214;
	case '5':	goto yy215;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy216;
	case ':':	goto yy213;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy212;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy184:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy216;
	case ':':	goto yy213;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy212;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy185:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy212;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy186:
	yych = *++in->cur;
	switch (yych) {
	case ' ':
		in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy13;
	case '/':
		in->yyt19 = in->yyt20 = NULL;
		in->yyt18 = in->yyt21 = in->cur;
		goto yy96;
	case ':':
		in->yyt3 = in->yyt15;
		in->yyt18 = in->cur;
		goto yy133;
	case '?':
		in->yyt19 = in->yyt20 = in->yyt23 = in->yyt24 = NULL;
		in->yyt18 = in->yyt21 = in->yyt22 = in->cur;
		goto yy59;
	default:	goto yy9;
	}
yy187:
	yych = *++in->cur;
	switch (yych) {
	case ']':	goto yy9;
	default:	goto yy218;
	}
yy188:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy219;
	case ':':	goto yy165;
	default:	goto yy9;
	}
yy189:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy220;
	case ':':	goto yy221;
	default:	goto yy9;
	}
yy190:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy222;
	case '1':	goto yy223;
	case '2':	goto yy224;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy225;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy226;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy191:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy227;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy192:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy222;
	case '1':	goto yy223;
	case '2':	goto yy224;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy225;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy226;
	default:	goto yy9;
	}
yy193:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy228;
	case ':':	goto yy192;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy227;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy194:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy228;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy227;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy195:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy227;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy196:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy141;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy197:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy229;
	default:	goto yy9;
	}
yy198:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy229;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy200;
	default:	goto yy9;
	}
yy199:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy229;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy200;
	case '5':	goto yy230;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy197;
	default:	goto yy9;
	}
yy200:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy229;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy197;
	default:	goto yy9;
	}
yy201:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy231;
	default:	goto yy9;
	}
yy202:
	yych = *++in->cur;
	switch (yych) {
	case '\n':
		in->yyt8 = in->cur;
		goto yy232;
	default:	goto yy9;
	}
yy203:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy159;
	default:	goto yy9;
	}
yy204:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy233;
	case ':':	goto yy205;
	default:	goto yy9;
	}
yy205:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy234;
	case ':':	goto yy235;
	default:	goto yy9;
	}
yy206:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy236;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy207:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy238;
	case ':':	goto yy237;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy236;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy208:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy238;
	case '5':	goto yy239;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy240;
	case ':':	goto yy237;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy236;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy209:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy240;
	case ':':	goto yy237;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy236;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy210:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy236;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy211:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy241;
	case '1':	goto yy242;
	case '2':	goto yy243;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy244;
	default:	goto yy9;
	}
yy212:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy245;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy213:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy206;
	case '1':	goto yy207;
	case '2':	goto yy208;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy209;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy210;
	default:	goto yy9;
	}
yy214:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy246;
	case ':':	goto yy213;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy245;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy215:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy246;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy245;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy216:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy245;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy217:
	++in->cur;
	if ((in->lim - in->cur) < 14) if (fill(in, 14) != 0) return 1;
	yych = *in->cur;
yy218:
	switch (yych) {
	case '!':
	case '$':
	case '&':
	case '\'':
	case '(':
	case ')':
	case '*':
	case '+':
	case ',':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case ':':
	case ';':
	case '=':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '_':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '~':	goto yy217;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy219:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy165;
	default:	goto yy9;
	}
yy220:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy247;
	case ':':	goto yy221;
	default:	goto yy9;
	}
yy221:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy248;
	case ':':	goto yy249;
	default:	goto yy9;
	}
yy222:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy250;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy223:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy252;
	case ':':	goto yy251;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy250;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy224:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy252;
	case '5':	goto yy253;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy254;
	case ':':	goto yy251;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy250;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy225:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy254;
	case ':':	goto yy251;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy250;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy226:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy250;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy227:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy255;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy228:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy255;
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy229:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy256;
	case '1':	goto yy257;
	case '2':	goto yy258;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy259;
	default:	goto yy9;
	}
yy230:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy229;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy197;
	default:	goto yy9;
	}
yy231:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy260;
	default:	goto yy9;
	}
yy232:
	yych = *++in->cur;
	switch (yych) {
	case '\n':
		taglist(&in->yyt34, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt33, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt32, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt31, in->tok, in->tok - 1, &in->tlp);
		in->yyt29 = in->yyt30 = NULL;
		goto yy261;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':
		taglist(&in->yyt31, in->tok, in->cur, &in->tlp);
		in->yyt29 = in->yyt30 = NULL;
		goto yy263;
	default:	goto yy9;
	}
yy233:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy265;
	case ':':	goto yy205;
	default:	goto yy9;
	}
yy234:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy266;
	case ':':	goto yy267;
	default:	goto yy9;
	}
yy235:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy268;
	case '1':	goto yy269;
	case '2':	goto yy270;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy271;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy272;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy236:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy273;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy237:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy268;
	case '1':	goto yy269;
	case '2':	goto yy270;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy271;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy272;
	default:	goto yy9;
	}
yy238:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy274;
	case ':':	goto yy237;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy273;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy239:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy274;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy273;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy240:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy273;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy241:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy275;
	default:	goto yy9;
	}
yy242:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy275;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy244;
	default:	goto yy9;
	}
yy243:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy275;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy244;
	case '5':	goto yy276;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy241;
	default:	goto yy9;
	}
yy244:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy275;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy241;
	default:	goto yy9;
	}
yy245:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy277;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy246:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy277;
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy247:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy278;
	case ':':	goto yy221;
	default:	goto yy9;
	}
yy248:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy279;
	case ':':	goto yy280;
	default:	goto yy9;
	}
yy249:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy281;
	case '1':	goto yy282;
	case '2':	goto yy283;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy284;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy285;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy250:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy286;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy251:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy281;
	case '1':	goto yy282;
	case '2':	goto yy283;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy284;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy285;
	default:	goto yy9;
	}
yy252:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy287;
	case ':':	goto yy251;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy286;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy253:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy287;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy286;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy254:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy286;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy255:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy192;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy256:
	yych = *++in->cur;
	switch (yych) {
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy257:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy259;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy258:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy259;
	case '5':	goto yy288;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy256;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy259:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy256;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy260:
	yych = *++in->cur;
	switch (yych) {
	case ' ':	goto yy289;
	default:	goto yy9;
	}
yy261:
	++in->cur;
	m1 = in->yyt1;
	m2 = in->yyt2;
	at = in->yyt3;
	u3 = in->yyt4;
	u4 = in->yyt9;
	hs3 = in->yyt10;
	hs4 = in->yyt11;
	r3 = in->yyt12;
	r4 = in->yyt5;
	s1 = in->yyt13;
	s2 = in->yyt14;
	u1 = in->yyt15;
	u2 = in->yyt16;
	hs1 = in->yyt17;
	hs2 = in->yyt18;
	r1 = in->yyt19;
	r2 = in->yyt20;
	p1 = in->yyt21;
	p2 = in->yyt22;
	p3 = in->yyt23;
	p4 = in->yyt24;
	q1 = in->yyt25;
	q2 = in->yyt6;
	p5 = in->yyt26;
	p6 = in->yyt27;
	q3 = in->yyt28;
	q4 = in->yyt7;
	v4 = in->yyt8;
	rp1 = in->yyt29;
	rp2 = in->yyt30;
	h1 = in->yyt31;
	h2 = in->yyt32;
	h3 = in->yyt33;
	h4 = in->yyt34;
	au = in->yyt13;
	of = in->yyt26;
	v3 = in->yyt8;
	if (in->yyt8 != NULL) v3 -= 8;
	v1 = in->yyt29;
	if (in->yyt29 != NULL) v1 -= 13;
	v2 = in->yyt29;
	if (in->yyt29 != NULL) v2 -= 5;
	st1 = in->yyt29;
	if (in->yyt29 != NULL) st1 -= 4;
	st2 = in->yyt29;
	if (in->yyt29 != NULL) st2 -= 1;
#line 98 "../../../benchmarks/submatch_dfa_aot/src/re2c/http_rfc7230.re"
	{
        if (st1) {
            OUT("version-1: ", v1, v2);
            OUT("status: ", st1, st2);
            OUT("reason: ", rp1, rp2);
        } else if (m1) {
            OUT("method: ", m1, m2);
            if (of) {
                OUT("path-3: ", p5, p6);
                if (q3) OUT("query-2: ", q3, q4);
            } else if (au) {
                if (p1) {
                    OUT("scheme: ", s1, s2);
                    if (u1) OUT("user-1: ", u1, u2);
                    OUT("host-1: ", hs1, hs2);
                    if (r1) OUT("port-1: ", r1, r2);
                    OUT("path-1: ", p1, p2);
                } else {
                    OUT("path-2: ", p3, p4);
                }
                if (q1) OUT("query-1: ", q1, q2);
            } else if (at) {
                if (u3) OUT("user-2: ", u3, u4);
                OUT("host-2: ", hs3, hs4);
                if (r3) OUT("port-2: ", r3, r4);
            } else {
                outc(out, '*');
            }
            OUT("version-2: ", v3, v4);
        }
        print_headers(out, in->tok, h1, h2, h3, h4);
        outc(out, '\n');

        taglistpool_clear(&in->tlp, in);
        goto loop;
    }
#line 8159 "gen/re2c/http_rfc7230-bc-tdfa1.c"
yy263:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':	goto yy263;
	case ':':
		taglist(&in->yyt32, in->tok, in->cur, &in->tlp);
		goto yy290;
	default:	goto yy9;
	}
yy265:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy205;
	default:	goto yy9;
	}
yy266:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy292;
	case ':':	goto yy267;
	default:	goto yy9;
	}
yy267:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy293;
	case ':':	goto yy294;
	default:	goto yy9;
	}
yy268:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy295;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy269:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy297;
	case ':':	goto yy296;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy295;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy270:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy297;
	case '5':	goto yy298;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy299;
	case ':':	goto yy296;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy295;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy271:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy299;
	case ':':	goto yy296;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy295;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy272:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy295;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy273:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy300;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy274:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy300;
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy275:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy301;
	case '1':	goto yy302;
	case '2':	goto yy303;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy304;
	default:	goto yy9;
	}
yy276:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy275;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy241;
	default:	goto yy9;
	}
yy277:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy213;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy278:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy221;
	default:	goto yy9;
	}
yy279:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy305;
	case ':':	goto yy280;
	default:	goto yy9;
	}
yy280:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy306;
	case ':':	goto yy307;
	default:	goto yy9;
	}
yy281:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy308;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy282:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy310;
	case ':':	goto yy309;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy308;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy283:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy310;
	case '5':	goto yy311;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy312;
	case ':':	goto yy309;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy308;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy284:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy312;
	case ':':	goto yy309;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy308;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy285:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy308;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy286:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy313;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy287:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy313;
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy288:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy256;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy289:
	yych = *++in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\n':
		in->yyt29 = in->yyt30 = in->cur;
		goto yy316;
	default:
		in->yyt29 = in->cur;
		goto yy314;
	}
yy290:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
	case ' ':	goto yy290;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		taglist(&in->yyt33, in->tok, in->cur, &in->tlp);
		goto yy317;
	default:
		taglist(&in->yyt33, in->tok, in->cur, &in->tlp);
		goto yy318;
	}
yy292:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy320;
	case ':':	goto yy267;
	default:	goto yy9;
	}
yy293:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy321;
	case ':':	goto yy322;
	default:	goto yy9;
	}
yy294:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy323;
	case '1':	goto yy324;
	case '2':	goto yy325;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy326;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy327;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy295:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy328;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy296:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy323;
	case '1':	goto yy324;
	case '2':	goto yy325;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy326;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy327;
	default:	goto yy9;
	}
yy297:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy329;
	case ':':	goto yy296;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy328;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy298:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy329;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy328;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy299:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy328;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy300:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy237;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy301:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy330;
	default:	goto yy9;
	}
yy302:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy330;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy304;
	default:	goto yy9;
	}
yy303:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy330;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy304;
	case '5':	goto yy331;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy301;
	default:	goto yy9;
	}
yy304:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy330;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy301;
	default:	goto yy9;
	}
yy305:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy332;
	case ':':	goto yy280;
	default:	goto yy9;
	}
yy306:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy333;
	case ':':	goto yy334;
	default:	goto yy9;
	}
yy307:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy335;
	case '1':	goto yy336;
	case '2':	goto yy337;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy338;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy339;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy308:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy340;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy309:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy335;
	case '1':	goto yy336;
	case '2':	goto yy337;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy338;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy339;
	default:	goto yy9;
	}
yy310:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy341;
	case ':':	goto yy309;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy340;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy311:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy341;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy340;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy312:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy340;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy313:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy251;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy314:
	++in->cur;
	if ((in->lim - in->cur) < 2) if (fill(in, 2) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\n':
		in->yyt30 = in->cur;
		goto yy316;
	default:	goto yy314;
	}
yy316:
	yych = *++in->cur;
	switch (yych) {
	case '\n':
		taglist(&in->yyt34, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt33, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt32, in->tok, in->tok - 1, &in->tlp);
		taglist(&in->yyt31, in->tok, in->tok - 1, &in->tlp);
		in->yyt1 = in->yyt2 = in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt8 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		goto yy261;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':
		taglist(&in->yyt31, in->tok, in->cur, &in->tlp);
		in->yyt1 = in->yyt2 = in->yyt3 = in->yyt4 = in->yyt5 = in->yyt6 = in->yyt7 = in->yyt8 = in->yyt9 = in->yyt10 = in->yyt11 = in->yyt12 = in->yyt13 = in->yyt14 = in->yyt15 = in->yyt16 = in->yyt17 = in->yyt18 = in->yyt19 = in->yyt20 = in->yyt21 = in->yyt22 = in->yyt23 = in->yyt24 = in->yyt25 = in->yyt26 = in->yyt27 = in->yyt28 = NULL;
		goto yy263;
	default:	goto yy9;
	}
yy317:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '\t':
	case ' ':	goto yy342;
	case '\n':
		in->yyt34 = in->yyt35;
		goto yy261;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':
		in->yyt34 = in->yyt35;
		taglist(&in->yyt31, in->tok, in->cur, &in->tlp);
		goto yy263;
	default:	goto yy9;
	}
yy318:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy344;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy317;
	case ' ':	goto yy346;
	default:	goto yy318;
	}
yy320:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy267;
	default:	goto yy9;
	}
yy321:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy348;
	case ':':	goto yy322;
	default:	goto yy9;
	}
yy322:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy349;
	case ':':	goto yy350;
	default:	goto yy9;
	}
yy323:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy351;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy324:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy353;
	case ':':	goto yy352;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy351;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy325:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy353;
	case '5':	goto yy354;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy355;
	case ':':	goto yy352;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy351;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy326:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy355;
	case ':':	goto yy352;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy351;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy327:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy351;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy328:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy356;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy329:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy356;
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy330:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy357;
	case '1':	goto yy358;
	case '2':	goto yy359;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy360;
	default:	goto yy9;
	}
yy331:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy330;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy301;
	default:	goto yy9;
	}
yy332:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy280;
	default:	goto yy9;
	}
yy333:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy361;
	case ':':	goto yy334;
	default:	goto yy9;
	}
yy334:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy362;
	case '1':	goto yy363;
	case '2':	goto yy364;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy365;
	case ':':	goto yy366;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy367;
	default:	goto yy9;
	}
yy335:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy368;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy336:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy370;
	case ':':	goto yy369;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy368;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy337:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy370;
	case '5':	goto yy371;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy372;
	case ':':	goto yy369;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy368;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy338:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy372;
	case ':':	goto yy369;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy368;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy339:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy368;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy340:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy373;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy341:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy373;
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy342:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
	case ' ':	goto yy342;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy317;
	default:	goto yy318;
	}
yy344:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':	goto yy344;
	case '\n':	goto yy374;
	case ' ':	goto yy346;
	default:	goto yy375;
	}
yy346:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy344;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy317;
	case ' ':	goto yy346;
	default:	goto yy376;
	}
yy348:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy377;
	case ':':	goto yy322;
	default:	goto yy9;
	}
yy349:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy378;
	case ':':	goto yy379;
	default:	goto yy9;
	}
yy350:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy380;
	case '1':	goto yy381;
	case '2':	goto yy382;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy383;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy384;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy351:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy385;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy352:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy380;
	case '1':	goto yy381;
	case '2':	goto yy382;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy383;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy384;
	default:	goto yy9;
	}
yy353:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy386;
	case ':':	goto yy352;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy385;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy354:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy386;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy385;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy355:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy385;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy356:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy296;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy357:
	yych = *++in->cur;
	switch (yych) {
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy358:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy360;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy359:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy360;
	case '5':	goto yy387;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy357;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy360:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy357;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy361:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy388;
	case ':':	goto yy334;
	default:	goto yy9;
	}
yy362:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy389;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy363:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy391;
	case ':':	goto yy390;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy389;
	default:	goto yy9;
	}
yy364:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy391;
	case '5':	goto yy392;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy393;
	case ':':	goto yy390;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy389;
	default:	goto yy9;
	}
yy365:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy393;
	case ':':	goto yy390;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy389;
	default:	goto yy9;
	}
yy366:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy394;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy367:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy389;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy368:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy395;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy369:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy394;
	default:	goto yy9;
	}
yy370:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy396;
	case ':':	goto yy369;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy395;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy371:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy396;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy395;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy372:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy395;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy373:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy309;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy374:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '\n':
		in->yyt34 = in->yyt35;
		goto yy261;
	case '!':
	case '#':
	case '$':
	case '%':
	case '&':
	case '\'':
	case '*':
	case '+':
	case '-':
	case '.':
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'G':
	case 'H':
	case 'I':
	case 'J':
	case 'K':
	case 'L':
	case 'M':
	case 'N':
	case 'O':
	case 'P':
	case 'Q':
	case 'R':
	case 'S':
	case 'T':
	case 'U':
	case 'V':
	case 'W':
	case 'X':
	case 'Y':
	case 'Z':
	case '^':
	case '_':
	case '`':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':
	case 'g':
	case 'h':
	case 'i':
	case 'j':
	case 'k':
	case 'l':
	case 'm':
	case 'n':
	case 'o':
	case 'p':
	case 'q':
	case 'r':
	case 's':
	case 't':
	case 'u':
	case 'v':
	case 'w':
	case 'x':
	case 'y':
	case 'z':
	case '|':
	case '~':
		in->yyt34 = in->yyt35;
		taglist(&in->yyt31, in->tok, in->cur, &in->tlp);
		goto yy263;
	default:	goto yy9;
	}
yy375:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
		taglist(&in->yyt34, in->tok, in->cur, &in->tlp);
		goto yy397;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy317;
	default:	goto yy318;
	}
yy376:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy399;
	case '\n':
		in->yyt35 = in->yyt34;
		taglist(&in->yyt35, in->tok, in->cur, &in->tlp);
		goto yy317;
	default:	goto yy318;
	}
yy377:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy322;
	default:	goto yy9;
	}
yy378:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy401;
	case ':':	goto yy379;
	default:	goto yy9;
	}
yy379:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy402;
	case ':':	goto yy403;
	default:	goto yy9;
	}
yy380:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy404;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy381:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy406;
	case ':':	goto yy405;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy404;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy382:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy406;
	case '5':	goto yy407;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy408;
	case ':':	goto yy405;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy404;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy383:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy408;
	case ':':	goto yy405;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy404;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy384:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy404;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy385:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy409;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy386:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy409;
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy387:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy357;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy388:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy334;
	default:	goto yy9;
	}
yy389:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy410;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy390:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy394;
	case ':':	goto yy256;
	default:	goto yy9;
	}
yy391:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy411;
	case ':':	goto yy390;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy410;
	default:	goto yy9;
	}
yy392:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy411;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy410;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy393:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy410;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy394:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy412;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy395:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy413;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy396:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy413;
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy397:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case '\t':
	case ' ':	goto yy397;
	case '\n':
		in->yyt35 = in->yyt34;
		goto yy374;
	default:	goto yy9;
	}
yy399:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':	goto yy399;
	case '\n':	goto yy374;
	case ' ':	goto yy414;
	default:	goto yy375;
	}
yy401:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy416;
	case ':':	goto yy379;
	default:	goto yy9;
	}
yy402:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy417;
	case ':':	goto yy418;
	default:	goto yy9;
	}
yy403:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy419;
	case '1':	goto yy420;
	case '2':	goto yy421;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy422;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy423;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy404:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy424;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy405:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy419;
	case '1':	goto yy420;
	case '2':	goto yy421;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy422;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy423;
	default:	goto yy9;
	}
yy406:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy425;
	case ':':	goto yy405;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy424;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy407:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy425;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy424;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy408:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy424;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy409:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy352;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy410:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy426;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy411:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy113;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy426;
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy412:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy427;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy413:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy369;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy414:
	++in->cur;
	if (in->lim <= in->cur) if (fill(in, 1) != 0) return 1;
	yych = *in->cur;
	switch (yych) {
	case 0x00:
	case 0x01:
	case 0x02:
	case 0x03:
	case 0x04:
	case 0x05:
	case 0x06:
	case 0x07:
	case 0x08:
	case '\v':
	case '\f':
	case '\r':
	case 0x0E:
	case 0x0F:
	case 0x10:
	case 0x11:
	case 0x12:
	case 0x13:
	case 0x14:
	case 0x15:
	case 0x16:
	case 0x17:
	case 0x18:
	case 0x19:
	case 0x1A:
	case 0x1B:
	case 0x1C:
	case 0x1D:
	case 0x1E:
	case 0x7F:	goto yy9;
	case '\t':	goto yy399;
	case '\n':	goto yy317;
	case ' ':	goto yy414;
	default:	goto yy376;
	}
yy416:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy379;
	default:	goto yy9;
	}
yy417:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy428;
	case ':':	goto yy418;
	default:	goto yy9;
	}
yy418:
	yych = *++in->cur;
	switch (yych) {
	case '0':	goto yy429;
	case '1':	goto yy430;
	case '2':	goto yy431;
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy432;
	case ':':	goto yy433;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy434;
	default:	goto yy9;
	}
yy419:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy435;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy420:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy437;
	case ':':	goto yy436;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy435;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy421:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy437;
	case '5':	goto yy438;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy439;
	case ':':	goto yy436;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy435;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy422:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy439;
	case ':':	goto yy436;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy435;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy423:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy435;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy424:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy440;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy425:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy440;
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy426:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy390;
	default:	goto yy9;
	}
yy427:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy256;
	case ']':	goto yy88;
	default:	goto yy9;
	}
yy428:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy441;
	case ':':	goto yy418;
	default:	goto yy9;
	}
yy429:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy442;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy430:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy444;
	case ':':	goto yy443;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy442;
	default:	goto yy9;
	}
yy431:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':	goto yy444;
	case '5':	goto yy445;
	case '6':
	case '7':
	case '8':
	case '9':	goto yy446;
	case ':':	goto yy443;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy442;
	default:	goto yy9;
	}
yy432:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy446;
	case ':':	goto yy443;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy442;
	default:	goto yy9;
	}
yy433:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy447;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy434:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy442;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy435:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy448;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy436:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy447;
	default:	goto yy9;
	}
yy437:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy449;
	case ':':	goto yy436;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy448;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy438:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy449;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy448;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy439:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy448;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy440:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy405;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy441:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy418;
	default:	goto yy9;
	}
yy442:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy450;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy443:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy447;
	case ':':	goto yy357;
	default:	goto yy9;
	}
yy444:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':	goto yy451;
	case ':':	goto yy443;
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy450;
	default:	goto yy9;
	}
yy445:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':	goto yy451;
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy450;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy446:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy450;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy447:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy452;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy448:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy453;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy449:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy453;
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy450:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy454;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy451:
	yych = *++in->cur;
	switch (yych) {
	case '.':	goto yy211;
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy454;
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy452:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy455;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy453:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy436;
	case ']':	goto yy186;
	default:	goto yy9;
	}
yy454:
	yych = *++in->cur;
	switch (yych) {
	case ':':	goto yy443;
	default:	goto yy9;
	}
yy455:
	yych = *++in->cur;
	switch (yych) {
	case '0':
	case '1':
	case '2':
	case '3':
	case '4':
	case '5':
	case '6':
	case '7':
	case '8':
	case '9':
	case 'A':
	case 'B':
	case 'C':
	case 'D':
	case 'E':
	case 'F':
	case 'a':
	case 'b':
	case 'c':
	case 'd':
	case 'e':
	case 'f':	goto yy357;
	case ']':	goto yy186;
	default:	goto yy9;
	}
}
#line 134 "../../../benchmarks/submatch_dfa_aot/src/re2c/http_rfc7230.re"

}
